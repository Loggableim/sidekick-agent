...
# per-profile log file (~/.sidekick/logs/mcp-stderr.log), tagged with the
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Sidekick MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Sidekick gateways know how to render.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Sidekick is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `sidekick mcp login {server_name}` (or delete the tokens "
            f"file under ~/.sidekick/mcp-tokens/ and restart). Do NOT retry "
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``sidekick tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Sidekick registry.
