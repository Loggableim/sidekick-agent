"""``sidekick slack ...`` CLI subcommands.

Today only ``sidekick slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ sidekick slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Sidekick. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Sidekick deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from cli.commands import slack_app_manifest
...
def slack_manifest_command(args) -> int:
    ...
    """
    name = getattr(args, "name", None) or "Sidekick"
    description = getattr(args, "description", None) or "Your Sidekick agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("SIDEKICK_HOME") or os.environ.get("HERMES_HOME") or str(Path.home() / ".hermes")) / "slack-manifest.json"
        else:
