"""
MCP Server Management CLI — ``sidekick mcp`` subcommand.

Implements ``sidekick mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.sidekick/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # sidekick_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  sidekick mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  sidekick mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  sidekick mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_sidekick_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: sidekick mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_sidekick_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `sidekick mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  sidekick mcp add <name> --url <endpoint>')
        _info('  sidekick mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `sidekick mcp remove` + `sidekick mcp add` to reconfigure auth.")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'sidekick mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``sidekick mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("sidekick mcp serve                              Run as MCP server")
        _info("sidekick mcp add <name> --url <endpoint>        Add an MCP server")
        _info("sidekick mcp add <name> --command <cmd>         Add a stdio server")
        _info("sidekick mcp add <name> --preset <preset>       Add from a known preset")
        _info("sidekick mcp remove <name>                      Remove a server")
        _info("sidekick mcp list                               List servers")
        _info("sidekick mcp test <name>                        Test connection")
        _info("sidekick mcp configure <name>                   Toggle tools")
        _info("sidekick mcp login <name>                       Re-authenticate OAuth")
        print()
