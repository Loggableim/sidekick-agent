...
def skill_matches_platform(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from runtime.skill_utils import skill_matches_platform as _impl
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_sidekick_home()}/.env manually."
...
def _parse_frontmatter(content: str) -> Tuple[Dict[str, Any], str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from runtime.skill_utils import parse_frontmatter
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.sidekick/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("SIDEKICK_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.sidekick/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``sidekick skills`` config UI). Default False
            filters out disabled skills.

    Returns:
        List of skill metadata dicts (name, description, category).
    """
    from runtime.skill_utils import get_external_skills_dirs, iter_skill_index_files
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_sidekick_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from cli.plugins import _get_disabled_plugins, get_plugin_manager
    ...
    if preprocess:
        try:
            from runtime.skill_preprocessing import preprocess_skill_content
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.sidekick/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `sidekick skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
