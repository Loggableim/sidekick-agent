...
Sidekick seeds its credential pool from many places:
    ...
    manual        — user ran `sidekick auth add`
...
unify here is **removal**:
    ...
    ``sidekick auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.sidekick/.env  → clear it
    ...
    """
    from sidekick_cli.config import get_env_path, remove_env_value
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Sidekick stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `sidekick auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    ...
    result = RemovalResult()
    oauth_file = get_sidekick_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Sidekick Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `sidekick login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `sidekick auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Sidekick auth store is not enough — _seed_from_singletons() would
    ...
    via ``sidekick auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from sidekick_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `sidekick auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `sidekick auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Sidekick stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `sidekick auth add copilot` to re-enable if needed.",
    ])
