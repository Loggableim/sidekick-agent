...
def get_nova_state_paths() -> NovaStatePaths:
    ...
    return NovaStatePaths(
        ...
        yolo=state_dir / "yolo.json",
    )


def load_nova_model_registry() -> dict[str, Any]:
    """Read the model map generated by Nova's canonical model starter."""
    payload = _read_json(get_nova_space_root() / "model_registry.json", {})
...
def load_nova_yolo_state() -> dict[str, Any]:
    """Return Nova's persisted autonomy override state.

    This is intentionally separate from the per-chat ``/api/session/yolo``
    flag. It governs Nova's autonomous runtime across restarts and spaces.
    """
    data = _read_json(get_nova_state_paths().yolo, {})
...
def set_nova_yolo_enabled(enabled: bool) -> dict[str, Any]:
    """Persist Nova's explicit full-autonomy override and audit the change."""
    state = {"enabled": bool(enabled), "updated_at": _now()}
    _write_json(get_nova_state_paths().yolo, state)
    _append_event(_new_event(
        "governance",
        status="completed",
        summary="Nova YOLO autonomy enabled" if state["enabled"] else "Nova YOLO autonomy disabled",
...
def guard_autonomous_action(action: dict[str, Any], autonomy_level: int | None = None) -> dict[str, Any]:
    ...
    if load_nova_yolo_state()["enabled"]:
        return {"allowed": True, "reason": "nova_yolo_override", "autonomy_level": level, "yolo_enabled": True}
...
def ensure_background_cron_jobs() -> dict[str, Any]:
    ...
    for module in (cron_jobs, runtime_cron_jobs):
        try:
            module.SIDEKICK_DIR = sidekick_home
    ...
    return {
        ...
        "ticker": "cli.web_server starts cron.scheduler.tick in a background thread every 60s",
        "written_scripts": written_scripts,
        "created": created,
        "updated": updated,
        "pruned": pruned,
...
def get_nova_status() -> dict[str, Any]:
    ...
    return {
        ...
        "model_registry": load_nova_model_registry(),
