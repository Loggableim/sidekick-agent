"""
Configuration management for Sidekick Agent.

Config files are stored in ~/.sidekick/ for easy access:
- ~/.sidekick/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.sidekick/.env         - API keys and secrets

This module provides:
- sidekick config          - Show current configuration
- sidekick config edit     - Open config in editor
- sidekick config set      - Set a specific value
- sidekick config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.sidekick/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``sidekick logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  sidekick config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_sidekick_home() / ".managed"
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade sidekick-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "sidekick update"
...
def format_managed_message(action: str = "modify this Sidekick installation") -> str:
    ...
    return (
        f"Cannot {action}: this Sidekick installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Sidekick."
    )
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container_mode_file = get_sidekick_home() / ".container-mode"
...
def _is_container() -> bool:
    ...
    When Sidekick runs in a container with volume-mounted config files, forcing
...
def ensure_sidekick_home():
    """Ensure ~/.sidekick directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_sidekick_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_sidekick_home_managed(home: Path):
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Sidekick-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Sidekick auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Sidekick sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Sidekick
        ...
        #  "/home/user/.sidekick/cache/documents:/output"]
        ...
        # bundled Sidekick image, which drops to the `sidekick` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Sidekick sends a stable profile-scoped userId to Camofox
    ...
    # Opt in via ``sidekick chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.sidekick/checkpoints/`` size (MB).  When
    ...
    # single read_file page exceeds these limits, Sidekick truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``sidekick kanban specify`` (single id or --all). Set a
        ...
        # model; override via `sidekick model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # e.g. `model · 68% · ~/projects/sidekick`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.sidekick/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.sidekick/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Sidekick feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Sidekick auto-pauses the goal and
    ...
    # cron daemon.
    #
    # See `sidekick curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.sidekick/skills/ into
        # ~/.sidekick/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `sidekick curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Sidekick Agent*" header.
    ...
    # stored approval from ~/.sidekick/shell-hooks-allowlist.json.
    ...
    # todos to ready, and fires `sidekick -p <assignee> chat -q ...` for
    ...
    # Logging — controls file logging to ~/.sidekick/logs/.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://sidekick-agent.sh/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `sidekick model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.sidekick/state.db.
    ...
    "sessions": {
        ...
        # default of ``sidekick sessions prune``.
        "retention_days": 90,
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'sidekick model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Sidekick auth store)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'sidekick' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Sidekick API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Sidekick API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Sidekick API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    """
    try:
        from runtime.skill_utils import discover_all_skill_config_vars, SKILL_CONFIG_PREFIX
    except Exception:
        return []
    ...
    try:
        ...
        # should never break `sidekick update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Sidekick versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``sidekick_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'sidekick setup' to create a valid config")]
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'sidekick doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `sidekick plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `sidekick curator status` has no
    ...
    #   3. Creates `~/.sidekick/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_sidekick_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_sidekick_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: sidekick config set <key> <value>")
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        if answer in {"y", "yes"}:
            ...
            config = load_config()
            try:
                from runtime.skill_utils import SKILL_CONFIG_PREFIX
            except Exception:
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _sidekick_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.sidekick/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.sidekick/config.yaml.
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.sidekick/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.sidekick/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.sidekick/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.sidekick/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.sidekick/.env and os.environ.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.sidekick/.env or environment."""
...
def redact_key(key: str) -> str:
    ...
    """
    from runtime.redact import mask_secret
...
def show_config():
    ...
    print(color("│              ⚕ Sidekick Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Skill config
    try:
        from runtime.skill_utils import discover_all_skill_config_vars, resolve_skill_config_values
    ...
    print(color("  sidekick config edit     # Edit config file", Colors.DIM))
    print(color("  sidekick config set <key> <value>", Colors.DIM))
    print(color("  sidekick setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: sidekick config set <key> <value>")
            print()
            print("Examples:")
            print("  sidekick config set model anthropic/claude-sonnet-4")
            print("  sidekick config set terminal.backend docker")
            print("  sidekick config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'sidekick config migrate' to add them")
        ...
        print("  sidekick config           Show current configuration")
        print("  sidekick config edit      Open config in editor")
        print("  sidekick config set <key> <value>   Set a config value")
        print("  sidekick config check     Check for missing/outdated config")
        print("  sidekick config migrate   Update config with new options")
        print("  sidekick config path      Show config file path")
        print("  sidekick config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``sidekick config`` UI so users
