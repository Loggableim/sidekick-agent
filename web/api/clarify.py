...
def sse_subscribe(session_id: str) -> queue.Queue:
    ...
    with _lock:
        # Remove stale subscribers before adding new ones (problem #10)
        _cleanup_stale_subscribers(session_id)
...
def sse_unsubscribe(session_id: str, q: queue.Queue) -> None:
    ...
    # Opportunistic cleanup after unsubscribe
    _cleanup_stale_subscribers(session_id)


def _cleanup_stale_subscribers(session_id: str | None = None) -> None:
    ...
    with _lock:
        ...
        for sid in targets:
            subs = _clarify_sse_subscribers.get(sid)
            if not subs:
                continue
            # Remove queues that are completely full (client stopped reading)
            subs[:] = [q for q in subs if q.qsize() < q.maxsize]
            if not subs:
                _clarify_sse_subscribers.pop(sid, None)
