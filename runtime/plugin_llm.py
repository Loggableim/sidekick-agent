...
Plugins built on Sidekick Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Sidekick
...
:class:`~sidekick_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Sidekick's main config
...
handles every provider, fallback chain, and per-task override Sidekick
supports.
"""
...
def _resolve_attribution(
    *,
    ...
    if provider_override:
        ...
        try:
            from runtime.auxiliary_client import _read_main_provider
    ...
    if isinstance(response_model, str) and response_model.strip():
        ...
        try:
            from runtime.auxiliary_client import _read_main_model
...
class PluginLlm:
    ...
    Instances are constructed by :class:`sidekick_cli.plugins.PluginContext`
    ...
    async def _invoke_async(
        ...
        *,
        ...
        from runtime.auxiliary_client import async_call_llm
        merged_extra = dict(extra_body or {})
        if profile_override:
            merged_extra.setdefault("metadata", {})["auth_profile"] = profile_override
