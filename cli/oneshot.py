...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"sidekick -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "sidekick -z: --toolsets all enables every toolset; "
    ...
    if unresolved:
        try:
            ...
            from cli.tools_config import _parse_enabled_flag
    ...
    if unknown:
        sys.stderr.write(f"sidekick -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "sidekick -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "sidekick -z: --toolsets did not contain any valid toolsets.\n"
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``sidekick -z`` / oneshot mode.

    Oneshot bypasses ``SidekickCLI._init_agent()``, so it must wire the SQLite
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("SIDEKICK_INFERENCE_PROVIDER") or os.getenv("HERMES_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
