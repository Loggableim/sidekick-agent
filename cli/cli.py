#!/usr/bin/env python3
"""
Sidekick Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Sidekick Agent, inspired by Claude Code.
...
try:
    from cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from runtime.usage_pricing import (
    CanonicalUsage,
    estimate_usage_cost,
    format_duration_compact,
    format_token_count_compact,
)
from runtime.markdown_tables import (
    is_table_divider,
    looks_like_table_row,
    realign_markdown_tables,
)
# NOTE: `from runtime.account_usage import ...` is deliberately NOT at module
# top — it transitively pulls the OpenAI SDK chain (~230 ms cold) and is only
# needed when the user runs `/limits`. Lazy-imported inside the handler below.
from cli.banner import _format_context_length, format_banner_version_label
...
from cli.browser_connect import (
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from runtime._compat.shim_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.sidekick/config.yaml (user config - preferred)
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Sidekick, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Sidekick - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and SidekickCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `sidekick model`).
    ...
    from cli.config import _expand_env_vars
...
# Initialize centralized logging early — agent.log + errors.log in ~/.sidekick/logs/.
...
try:
    from cli.config import print_config_warnings
...
try:
    from cli.skin_engine import init_skin_from_config
...
try:
    from runtime.display import set_tool_preview_max_len
...
try:
    from runtime.auxiliary_client import neuter_async_httpx_del
...
from cli.banner import build_welcome_banner
from cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from cli.callbacks import prompt_for_secret
...
def _run_cleanup():
    ...
    try:
        from runtime.auxiliary_client import shutdown_cached_clients
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run sidekick -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"sidekick-{short_id}"
    branch_name = f"sidekick/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    if session_db is None:
        return
    try:
        from cli.config import load_config as _load_full_config
        from runtime._compat.shim_constants import get_sidekick_home as _get_sidekick_home
        _sidekick_home_maint = _get_sidekick_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_sidekick_home_maint / "sessions"
                )
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("sidekick-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    ...
    orphaned = [
        ...
        and (b.startswith("sidekick/sidekick-") or b.startswith("pr-"))
    ]
...
from runtime._compat.shim_constants import is_termux as _is_termux_environment
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ SIDEKICK - AI Agent Framework"
        tiny_line = "⚕ SIDEKICK"
    else:
        agent_name = _skin.get_branding("agent_name", "Sidekick Agent") if _skin else "Sidekick Agent"
...
from runtime.skill_commands import (
    scan_skill_commands,
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.sidekick/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.sidekick/config.yaml on first use)
        config_path.parent.mkdir(parents=True, exist_ok=True)
        
        # Save back atomically while preserving comments, ordering, quotes, and
        # readable Unicode in user-edited config.yaml.
        from shared.utils import atomic_roundtrip_yaml_update
...
class SidekickCLI:
    """
    Interactive CLI for the Sidekick Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from cli.runtime_provider import _auto_detect_local_model
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Sidekick'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from cli.models import copilot_model_api_mode, normalize_copilot_model_id
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Sidekick")
            ...
            w = shutil.get_terminal_size().columns
            fill = w - 2 - SidekickCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: sidekick setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: sidekick setup")
            return False
        ...
        # `sidekick chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `sidekick auth add openai-codex`
        # without `sidekick model`), fall back to the provider's first catalog
        # model so the API call doesn't fail with "model must be non-empty".
        if not self.model and resolved_provider:
            try:
                from cli.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from cli.models import resolve_fast_mode_overrides
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (sidekick sessions list).{_RST}")
                return False
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   Sidekick needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Sidekick: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Sidekick: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.sidekick/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from cli.clipboard import save_clipboard_image

        img_dir = get_sidekick_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: sidekick --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Sidekick config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_sidekick_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from cli.clipboard import has_clipboard_image
    ...
    def _fast_command_available(self) -> bool:
        try:
            from cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from cli.commands import COMMANDS_BY_CATEGORY

        try:
            from cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Sidekick!{_RST}")
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from cli.main import _relative_time
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        """
        from runtime._compat.shim_state import format_session_db_unavailable
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `sidekick gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `sidekick sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `sidekick sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.sidekick/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``sidekick --resume <id>``
        ...
        saved_dir = get_sidekick_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"sidekick_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: sidekick --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `sidekick model` and gateway pickers).
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cli.model_switch import switch_model, parse_model_flags, list_authenticated_providers
        from cli.providers import get_label
    ...
    def _handle_gquota_command(self, cmd_original: str) -> None:
        ...
        try:
            from runtime.google_oauth import get_valid_access_token, GoogleOAuthError, load_credentials
            from runtime.google_code_assist import retrieve_user_quota, CodeAssistError
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        try:
            from cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from cli.kanban import run_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_sidekick_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from cli.skin_engine import get_active_skin
            ...
            from cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_sidekick_home()}/plugins/ to get started.")
                else:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Sidekick keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_sidekick_home()}/skins/\n")
            return
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Sidekick is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Sidekick is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Sidekick is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        with self._busy_command("Compressing context..."):
            try:
                from runtime.model_metadata import estimate_request_tokens_rough
    ...
    def _handle_debug_command(self):
        ...
        from cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if rl_state and rl_state.has_data:
            from runtime.rate_limit_tracker import format_rate_limit_display
            print()
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.sidekick/skills/ and queue a note for the
        next user turn.
        ...
        """
        try:
            from runtime.skill_commands import reload_skills
    ...
    def _on_tool_gen_start(self, tool_name: str) -> None:
        ...
        from runtime.display import get_tool_emoji
    ...
    def _on_tool_progress(self, event_type: str, function_name: str = None, preview: str = None, function_args: dict = None, **kwargs):
        ...
        if event_type == "tool.completed":
            ...
            if function_name and self.tool_progress_mode in {"all", "new"}:
                ...
                try:
                    from runtime.display import get_cute_tool_message
        ...
        if function_name and not function_name.startswith("_"):
            ...
            from runtime.display import get_tool_preview_max_len
    ...
    def _on_tool_start(self, tool_call_id: str, function_name: str, function_args: dict):
        ...
        try:
            from runtime.display import capture_local_edit_snapshot
    ...
    def _on_tool_complete(self, tool_call_id: str, function_name: str, function_args: dict, function_result: str):
        ...
        try:
            from runtime.display import render_edit_diff_with_delta
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "sidekick_voice"), exist_ok=True)
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        if images:
            try:
                from runtime.image_routing import (
        ...
        if isinstance(message, str) and "@" in message:
            try:
                from runtime.context_references import preprocess_context_references
        ...
        try:
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _sidekick_home / "interrupt_debug.log"
            ...
            try:
                from runtime.auxiliary_client import cleanup_stale_async_clients
            ...
            if response and result and not result.get("failed") and not result.get("partial"):
                try:
                    from runtime.title_generator import maybe_auto_title
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  sidekick --resume {self.session_id}")
            if session_title:
                print(f"  sidekick -c \"{session_title}\"")
            print()
            ...
            try:
                from cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        # Only show when the user explicitly configured Honcho for Sidekick
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Sidekick Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Sidekick Agent! Type your message or /help for commands."
        ...
        # after an OpenClaw→Sidekick migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from cli.config import get_config_path as _get_cfg_path_resid
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Sidekick binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Sidekick Agent")
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if line_count >= 5 and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _sidekick_home / "pastes"
        ...
        # Dynamic prompt: shows Sidekick symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Sidekick needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Sidekick needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Sidekick needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = SidekickCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the sidekick process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: sidekick setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    Sidekick Agent CLI - Interactive AI Assistant
    ...
    Examples:
        ...
        python cli.py --skills sidekick-dev,github-auth
    ...
    if gateway:
        ...
        print("Starting Sidekick Gateway (messaging platforms)...")
    ...
    # mode registers its own inside SidekickCLI.run(), but `-q` runs
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Sidekick" box, no tool-gen
            ...
            # Single-query mode (`sidekick chat -q "…"`): skip the welcome
