...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m sidekick_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Sidekick_Gateway"
_TASK_DESCRIPTION = "Sidekick Agent Gateway - Messaging Platform Integration"
...
def get_task_name() -> str:
    ...
    from cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    script_dir = Path(get_sidekick_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    prog_args = [python_path, "-m", "sidekick_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m sidekick_cli.main gateway run --replace``
    ...
    log_dir = Path(get_sidekick_home()) / "logs"
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from cli.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from cli.config import get_sidekick_home
        print(f"    type {Path(get_sidekick_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_sidekick_home()).resolve()}\\logs\\gateway-stdio.log")
...
def stop() -> None:
    ...
    _assert_windows()
    from cli.gateway import kill_gateway_processes
