...
`$SIDEKICK_HOME/plugins/model-providers/`) the first time anything calls
...
User plugins at `$SIDEKICK_HOME/plugins/model-providers/<name>/` override
bundled plugins of the same name — last-writer-wins in
`register_provider()`. Drop a file there to replace a built-in.

## Adding a new provider

1. Create `providers/bundled/<your_provider>/__init__.py`:
...
2. Create `providers/bundled/<your_provider>/plugin.yaml`:
