...
from runtime._compat.shim_constants import get_sidekick_home, get_skills_dir, is_wsl
from typing import Optional

from runtime.skill_utils import (
...
DEFAULT_AGENT_IDENTITY = (
    "You are Sidekick, an intelligent AI assistant. "
...
KANBAN_GUIDANCE = (
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `sidekick kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Sidekick WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Sidekick itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Sidekick runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Sidekick itself is running. The host OS, home, and cwd "
                f"of the Sidekick process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Sidekick "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_sidekick_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.sidekick/skills/`` directory.  External dirs
