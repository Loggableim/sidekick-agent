...
Supports OAuth device code flows (OpenAI Codex) and
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.code == "insufficient_credits":
        return (
            ...
            "Top up/renew credits, then retry."
        )
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared store) don't share reentrancy
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
