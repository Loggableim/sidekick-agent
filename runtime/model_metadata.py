...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Sidekick also honours SIDEKICK_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("SIDEKICK_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run Sidekick Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Sidekick from probing-down to
...
def _get_context_cache_path() -> Path:
    ...
    return get_sidekick_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Sidekick grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if custom_providers and base_url and model:
        try:
            from sidekick_cli.config import get_custom_provider_context_length
    ...
    if provider == "bedrock" or (
        ...
        try:
            from runtime.bedrock_adapter import get_bedrock_context_length
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from sidekick_cli.models import get_copilot_model_context
    ...
    if effective_provider:
        from runtime.models_dev import lookup_models_dev_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Sidekick sends to providers:
