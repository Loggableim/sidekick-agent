...
def _resolve_download_timeout() -> float:
    env_val = (os.getenv("SIDEKICK_VISION_DOWNLOAD_TIMEOUT") or os.getenv("HERMES_VISION_DOWNLOAD_TIMEOUT", "")).strip()
    if env_val:
        try:
            ...
            pass
    try:
        from cli.config import cfg_get, load_config
        cfg = load_config()
...
def _handle_vision_analyze(args: Dict[str, Any], **kw: Any) -> Awaitable[str]:
    ...
    try:
        from runtime.auxiliary_client import _read_main_provider, _read_main_model
        from runtime.image_routing import decide_image_input_mode
