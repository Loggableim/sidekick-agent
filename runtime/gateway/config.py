...
class Platform(Enum):
    ...
    def _missing_(cls, value):
        ...
        try:
            from runtime.gateway.platform_registry import platform_registry
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_sidekick_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.sidekick/config.yaml (primary user-facing config)
    3. ~/.sidekick/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_sidekick_home()
    gw_data: dict = {}
...
def _validate_gateway_config(config: "GatewayConfig") -> None:
    ...
    try:
        from sidekick_cli.auth import has_usable_secret
    except ImportError:
