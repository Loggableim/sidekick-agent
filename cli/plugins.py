...
from shared.utils import env_var_enabled
...
def get_bundled_plugins_dir() -> Path:
    ...
    """
    env_override = os.getenv("SIDEKICK_BUNDLED_PLUGINS") or os.getenv("HERMES_BUNDLED_PLUGINS")
    if env_override:
...
_PLUGINS_DEBUG = (os.getenv("SIDEKICK_PLUGINS_DEBUG") or os.getenv("HERMES_PLUGINS_DEBUG", "")).strip().lower() in {
    "1", "true", "yes", "on",
}
...
class PluginManifest:
    ...
    #              in ~/.sidekick/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``sidekick plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def llm(self) -> Any:
        ...
        if self._llm is None:
            from runtime.plugin_llm import PluginLlm
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``sidekick honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``sidekick <subcommand>``
    ...
    def register_image_gen_provider(self, provider) -> None:
        ...
        tool calls.
        """
        from runtime.image_gen_provider import ImageGenProvider
        from runtime.image_gen_registry import register_provider
    ...
    def register_skill(
        ...
        ``~/.sidekick/skills/`` tree and is **not** listed in the system
        ...
        """
        from runtime.skill_utils import _NAMESPACE_RE
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to sidekick_cli/. Two layouts are
        ...
        # 2. User plugins (~/.sidekick/plugins/)
        user_dir = get_sidekick_home() / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with sidekick and must
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `sidekick plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``sidekick tools`` TUI so plugin-provided toolsets appear
