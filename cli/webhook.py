"""sidekick webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    sidekick webhook subscribe <name> [options]
    sidekick webhook list
    sidekick webhook remove <name>
    sidekick webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.sidekick/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _sidekick_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: sidekick gateway run
"""
...
def webhook_command(args):
    """Entry point for 'sidekick webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: sidekick webhook {subscribe|list|remove|test}")
        print("Run 'sidekick webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (sidekick gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: sidekick webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from sidekick webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (sidekick gateway run)")
