...
class DiagnosticAction:
    ...
    * ``cli_hint`` — print/copy a shell command (e.g.
      ``sidekick -p <profile> auth``). No HTTP side effect.
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Verify profile: sidekick -p {assignee} doctor",
            payload={"command": f"sidekick -p {assignee} doctor"},
            suggested=True,
        ))
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Fix profile auth: sidekick -p {assignee} auth",
            payload={"command": f"sidekick -p {assignee} auth"},
        ))
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: sidekick kanban log {task_id}",
                payload={"command": f"sidekick kanban log {task_id}"},
                suggested=True,
            ))
    ...
    * Dispatcher is misconfigured (wrong board, wrong HERMES_HOME).
    ...
    actions = [
        ...
        DiagnosticAction(
            ...
            payload={"command": "sidekick kanban diagnostics"},
        ),
    ]
