"""sidekick hooks — inspect and manage shell-script hooks.

Usage::

    sidekick hooks list
    sidekick hooks test <event> [--for-tool X] [--payload-file F]
    sidekick hooks revoke <command>
    sidekick hooks doctor

Consent records live under ``~/.sidekick/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.sidekick/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``sidekick hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: sidekick hooks {list|test|revoke|doctor}")
        print("Run 'sidekick hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    from runtime import shell_hooks

    specs = shell_hooks.iter_configured_hooks(load_config())

    if not specs:
        print("No shell hooks configured in ~/.sidekick/config.yaml.")
        print("See `sidekick hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `sidekick hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `sidekick hooks test` and `sidekick hooks doctor`
...
def _cmd_test(args) -> None:
    from cli.config import load_config
    from cli.plugins import VALID_HOOKS
    from runtime import shell_hooks
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Sidekick wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `sidekick hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `sidekick hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `sidekick hooks doctor`.")
