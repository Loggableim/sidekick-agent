"""Slash command definitions and autocomplete for the Sidekick CLI.
...
from shared.utils import is_truthy_value
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Sidekick config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Sidekick works on across turns until achieved", "Session",
               args_hint="[text | pause | resume | clear | status]"),
    CommandDef("plan", "Create or revise a server-enforced implementation plan", "Session",
               args_hint="<request>", subcommands=("revise", "reject"), cli_only=True),
    CommandDef("execute", "Explicitly approve and execute the current plan", "Session",
               args_hint="[plan-id]", cli_only=True),
    ...
    CommandDef("busy", "Control what Enter does while Sidekick is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.sidekick/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Sidekick Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`sidekick_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/sidekick`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        from runtime.skill_utils import get_disabled_skill_names
    ...
    try:
        from runtime.skill_commands import get_skill_commands
        ...
        # ``sidekick skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``sidekick skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``sidekick skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/sidekick <verb>`` subcommand.
    ...
    can still reach them via ``/sidekick <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/sidekick`` is always reserved as the first entry so the
    legacy ``/sidekick <subcommand>`` form keeps working for anything that
    ...
    seen: set[str] = set()
            ...
            # Reserve /sidekick as the catch-all top-level command.
    entries.append(("sidekick", "Talk to Sidekick or run a subcommand", "[subcommand] [args]"))
    seen.add("sidekick")
...
def slack_app_manifest(request_url: str = "https://sidekick-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /sidekick handler.

    Maps both canonical names and aliases so /sidekick bg do stuff works
    the same as /sidekick background do stuff.

    Plugin-registered slash commands are included so ``/sidekick <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
