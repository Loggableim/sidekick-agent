...
Usage examples::
    ...
    sidekick logs                    # last 50 lines of agent.log
    sidekick logs -f                 # follow agent.log in real time
    sidekick logs errors             # last 50 lines of errors.log
    sidekick logs gateway -n 100    # last 100 lines of gateway.log
    sidekick logs --level WARNING    # only WARNING+ lines
    sidekick logs --session abc123   # filter by session ID substring
    sidekick logs --component tools  # only tool-related lines
    sidekick logs --since 1h         # lines from the last hour
    sidekick logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if follow:
        print(f"--- {display_sidekick_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_sidekick_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_sidekick_home()}/logs/")
        return

    print(f"Log files in {display_sidekick_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'sidekick chat' to generate logs)")
