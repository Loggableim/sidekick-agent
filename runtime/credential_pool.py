...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``sidekick model`` / ``sidekick auth`` which
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                from runtime.anthropic_adapter import refresh_anthropic_oauth_pure

                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("sidekick_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if entry.source == "claude_code":
                    try:
                        from runtime.anthropic_adapter import _write_claude_code_credentials
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("sidekick_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Sidekick profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `sidekick model` / `sidekick auth` after a 429/401,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `sidekick auth remove <provider> <N>` is stable across all source types.
    try:
        from cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Sidekick PKCE)
        ...
        from runtime.anthropic_adapter import read_claude_code_credentials, read_sidekick_oauth_credentials

        for source_name, creds in (
            ("sidekick_pkce", read_sidekick_oauth_credentials()),
        ...
        # Sidekick auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.sidekick/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `sidekick auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `sidekick auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Sidekick auth store.  Without this gate the removal is instantly
        ...
        # Sidekick owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `sidekick auth openai-codex`.
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.sidekick/.env over os.environ — the user's config file is the
    # authoritative source for Sidekick credentials. Stale env vars from parent
    ...
    # Honour user suppression — `sidekick auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.sidekick/.env.
    ...
    try:
        from cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.sidekick/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "sidekick_pkce"}
        )
    ]
