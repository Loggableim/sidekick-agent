...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Sidekick starts and finds ``~/.openclaw/``.

    Points users at ``sidekick claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``sidekick claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Sidekick, run "
        "`sidekick claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `sidekick claw cleanup` (renames it to ~/.openclaw.pre-migration — "
