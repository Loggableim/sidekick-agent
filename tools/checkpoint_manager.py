...
CHECKPOINT_BASE = get_sidekick_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("SIDEKICK_CHECKPOINT_TIMEOUT") or os.getenv("HERMES_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(
    ...
    The shared store is internal Sidekick infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``sidekick checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `sidekick checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
# ---------------------------------------------------------------------------
# Public helpers for `sidekick checkpoints` CLI
# ---------------------------------------------------------------------------
