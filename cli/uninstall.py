"""
Sidekick Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.sidekick/ (configs, sessions, logs)
"""
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``sidekick gateway run`` processes
    ...
    try:
        from cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from cli.gateway import get_launchd_plist_path
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.sidekick/ (configs, data, logs)
    ...
    print(color("│            ⚕ Sidekick Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Sidekick data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Sidekick code but keep your configuration and data.")
    ...
    print()
    print("Thank you for using Sidekick Agent! ⚕")
    print()
