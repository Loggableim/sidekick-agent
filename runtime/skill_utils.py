...
from runtime._compat.shim_constants import get_config_path, get_skills_dir
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.sidekick/skills/`` are silently skipped.
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.sidekick/skills/`` first, then external.
