...
def _is_compression_marker_message(message):
    if not isinstance(message, dict):
        return False
    role = message.get("role")
    if not role or role == "tool":
        return False
    text = _content_text(message.get("content", ""), part_types={"text", "input_text", "output_text"})
    norm = " ".join(text.split()).strip().lower()
    if not norm:
        return False
    return (
        norm.startswith("[context compaction")
        or norm.startswith("context compaction")
        or "context was auto-compressed" in norm
        or norm.startswith("[your active task list was preserved across context compression]")
    )
...
def visible_messages_for_anchor(messages, *, auto_compression: bool = False):
    ...
    for message in messages or []:
        if not isinstance(message, dict):
            continue
        if _is_compression_marker_message(message):
            continue
        role = message.get("role")
        if not role or role == "tool":
            continue
