...
from web.api.turn_journal import (
...
def _state_db_row_to_sidecar(row: dict) -> dict:
    try:
        from web.api.agent_sessions import normalize_agent_session_source
    except Exception:
...
def repair_safe_session_recovery(session_dir: Path, state_db_path: Path | None = None) -> dict:
    ...
    if sidecar_repair.get('materialized'):
        try:
            from web.api.models import _write_session_index
...
def _main() -> int:
    parser = argparse.ArgumentParser(description="Audit Sidekick session recovery state")
    ...
    parser.add_argument("--state-db", type=Path, default=None, help="optional Sidekick state.db path")
