...
The "default" profile is ``~/.sidekick`` itself — backward compatible,
zero migration needed.

Usage::

    sidekick profile create coder          # fresh profile + bundled skills
    sidekick profile create coder --clone  # also copy config, .env, SOUL.md, skills
    sidekick profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    sidekick -p coder chat                 # or via flag
    sidekick profile use coder             # set as sticky default
    sidekick profile delete coder          # remove profile + alias + service
"""
...
# default profile (``~/.sidekick``).  Named profiles never contain these
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    is_default_source = source_resolved == _get_default_sidekick_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.sidekick) profile.
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_sidekick_home() / "active_profile"
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, sidekick subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``sidekick profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_sidekick_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``sidekick update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.sidekick)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.sidekick/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `sidekick update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`sidekick profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `sidekick update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.sidekick).\n"
            "To remove everything, use: sidekick uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.sidekick/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: sidekick profile create {canon}"
        )
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: sidekick profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.sidekick itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.sidekick). "
            "Specify a different name: sidekick profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_sidekick_home() / "honcho.json",
