"""Sidekick CLI skin/theme engine.

A data-driven skin system that lets users customize the CLI's visual appearance.
Skins are defined as YAML files in ~/.sidekick/skins/ or as built-in presets.
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Sidekick Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Sidekick "       # Response box header label
...
.. code-block:: python
    ...
    print(skin.get_branding("agent_name"))  # "Sidekick Agent"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.sidekick/skins/

BUILT-IN SKINS
==============

- ``default`` — Classic Sidekick gold/kawaii (the current look)
...
Drop a YAML file in ``~/.sidekick/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces SIDEKICK_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces SIDEKICK_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Sidekick — gold & starlight, powered by NOVA",
        "colors": {
...
def _skins_dir() -> Path:
    ...
    return get_sidekick_home() / "skins"
