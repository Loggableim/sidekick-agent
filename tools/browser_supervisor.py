...
One ``CDPSupervisor`` runs per Sidekick ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "sidekick-dialog-bridge.invalid"
...
(() => {
  if (window.__sidekickDialogBridgeInstalled) return;
  window.__sidekickDialogBridgeInstalled = true;
  const ENDPOINT = "http://sidekick-dialog-bridge.invalid/";
