...
- ``sidekick_cli.providers``        -- canonical provider identity + overlays
- ``sidekick_cli.model_normalize``  -- per-provider name formatting
...
from runtime.models_dev import (
    ModelCapabilities,
    ModelInfo,
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``sidekick config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'sidekick model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'sidekick doctor' — config issues detected:"
    ...
    # Mirrors the same logic in sidekick_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from sidekick_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`sidekick_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from cli.models import fetch_openrouter_models
