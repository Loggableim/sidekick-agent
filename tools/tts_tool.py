...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.sidekick/config.yaml``. Sidekick
...
Configuration is loaded from ~/.sidekick/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    """
    try:
        from cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import sidekick_xai_user_agent
...
def _get_default_output_dir() -> str:
    ...
    return str(get_sidekick_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.sidekick/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.sidekick/config.yaml.
    ...
    try:
        ...
        logger.debug("sidekick_cli.config not available, using default TTS config")
        return {}
...
# custom voice-cloning scripts, etc.) into Sidekick without any Python code
...
#           output_format: wav
#
# Sidekick writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__SIDEKICK_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Sidekick actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": sidekick_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Sidekick caches Piper voice models.

    Resolves to ``~/.sidekick/cache/piper-voices/`` under the active
    SIDEKICK_HOME so voice downloads follow profile boundaries.
    """
    from runtime._compat.shim_constants import get_sidekick_dir
    root = Path(get_sidekick_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.sidekick/config.yaml (tts: section).
    ...
    platform = get_session_env("SIDEKICK_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'sidekick-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run sidekick setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'sidekick setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'sidekick tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_sidekick_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
