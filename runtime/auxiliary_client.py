...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.sidekick/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Sidekick triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://sidekick-agent.sh",
...
NOUS_EXTRA_BODY = {"tags": ["product=sidekick-agent"]}
...
_AUTH_JSON_PATH = get_sidekick_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Sidekick Agent)",
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        if temperature is not None:
            from runtime.anthropic_adapter import _forbids_sampling_params
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.sidekick/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Sidekick auth store.
    ...
    try:
        from cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from runtime.nous_rate_guard import nous_rate_limit_remaining
    ...
    model = _NOUS_MODEL
    try:
        from cli.models import get_nous_recommended_aux_model
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `sidekick setup` or `sidekick model` and enter a valid http(s) base URL."
        ) from exc
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second sidekick
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``sidekick config aux reset``)."""
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "anthropic":
            from runtime.anthropic_adapter import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `sidekick model` but the
    #    old OPENAI_BASE_URL lingers in ~/.sidekick/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: sidekick model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.sidekick/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from runtime.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        try:
            from runtime.model_metadata import _infer_provider_from_url
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: sidekick auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: sidekick model)")
                return None, None
    ...
    try:
        from cli.runtime_provider import _get_named_custom_provider
    ...
    if pconfig.auth_type == "external_process":
        ...
        try:
            from runtime.bedrock_adapter import has_aws_credentials, resolve_bedrock_region
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=sidekick-agent"])
    if merged_extra:
