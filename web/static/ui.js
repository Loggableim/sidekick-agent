...
function _clearQueuePanelDom(sessionId){
  ...
  const pillOuter=pill&&pill.parentElement;
  if(card){
    card.classList.remove('visible');
    if(card.dataset) card.dataset.queueSessionId='';
  }
  if(inner) inner.innerHTML='';
  const msgs=document.getElementById('messages');
  if(msgs){
    msgs.classList.remove('queue-open');
    if(msgs.style) msgs.style.removeProperty('--queue-card-height');
  }
  if(pillOuter) pillOuter.classList.remove('show');
  if(pill) pill.onclick=null;
  if(sessionId!=null) delete _queueRenderKeys[sessionId];
}
...
function workflowRefreshSubagentBadge(force){
  ...
  if(badge){badge.hidden=secondary;badge.disabled=false;badge.classList.remove('subagents-state-loading','subagents-state-active','subagents-state-paused','subagents-state-empty','subagents-state-offline');badge.classList.add(stateClass);badge.setAttribute('aria-busy',renderState.loading?'true':'false');if(!secondary)badge.setAttribute('aria-label',title);}
  if(value){value.textContent=label;value.hidden=secondary;}
}
...
function _renderQueueChips(sid){
  ...
  const renderedSid=String(card.dataset&&card.dataset.queueSessionId||'');
  const nextSid=String(sid||'');
  if(renderedSid!==nextSid) delete _queueRenderKeys[sid];
  const q=_getSessionQueue(sid,false);
...
function updateQueueBadge(sessionId){
  ...
  const card=document.getElementById('queueCard');
  const renderedSid=String(card&&card.dataset&&card.dataset.queueSessionId||'');
  if(count>0&&activeSid&&sid===activeSid){
    ...
    // Clear the shared queue UI when the active session changes, or when the
    // currently-rendered queue belongs to this session and the queue became empty.
    if(!activeSid || sid===activeSid || renderedSid===String(sid||'')){
      _clearQueuePanelDom(sid);
...
function _purgeStaleStates(all, maxBytes){
  if(!all||typeof all!=='object') return {};
  const now=Date.now();
  const stale=Object.entries(all).filter(([_,e])=>e&&e.updated_at&&now-e.updated_at>10*60*1000);
  stale.forEach(([k])=>delete all[k]);
  // If still too large, drop the largest entries first (by serialized size estimate)
  if(maxBytes>0){
    const serialized=JSON.stringify(all);
    if(serialized.length>maxBytes){
      const entries=Object.entries(all).map(([k,v])=>({k,v,sz:JSON.stringify(v).length})).sort((a,b)=>b.sz-a.sz);
      while(entries.length){
        delete all[entries.shift().k];
        if(JSON.stringify(all).length<=maxBytes) break;
      }
    }
  }
  return all;
...
function saveInflightState(sid, state){
  ...
  try{
    ...
    if(serialized.length>1.5*1024*1024){
      ...
      if(pruned.length>1.5*1024*1024) return; // still too big, give up
      localStorage.setItem(INFLIGHT_STATE_KEY, pruned);
    } else {
      localStorage.setItem(INFLIGHT_STATE_KEY, serialized);
    }
  }catch(_){ }
}
...
function syncTopbar(){
  if(!S.session){
    document.title=window._botName||'Nova';
...
function renderMessages(options){
  ...
  if(!S.busy && (!S.toolCalls||!S.toolCalls.length)){
    ...
    S.messages.forEach(m=>{
      if(!m) return;
      // OpenAI / Sidekick CLI format: role=tool with tool_call_id
      if(m.role==='tool'){
