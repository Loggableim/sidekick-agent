...
This module provides the glue:
    - ``SidekickTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Sidekick Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses SIDEKICK_HOME so each profile gets its own OAuth tokens.
    Layout: ``SIDEKICK_HOME/mcp-tokens/``
    """
    try:
        from runtime._compat.shim_constants import get_sidekick_home
        base = Path(get_sidekick_home())
    except ImportError:
        base = Path(os.environ.get("SIDEKICK_HOME", str(Path.home() / ".sidekick")))
...
# ---------------------------------------------------------------------------
# SidekickTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class SidekickTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        SIDEKICK_HOME/mcp-tokens/<server_name>.json         -- tokens
        SIDEKICK_HOME/mcp-tokens/<server_name>.client.json   -- client info
        SIDEKICK_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Sidekick records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Sidekick.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Sidekick Agent")
    scope = cfg.get("scope")
