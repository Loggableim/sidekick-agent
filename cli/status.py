"""
Status command for sidekick CLI.

Shows the status of all Sidekick Agent components.
"""
...
from cli.auth import AuthError, resolve_provider
from cli.colors import Colors, color
from cli.config import get_env_path, get_env_value, get_sidekick_home, load_config
from cli.models import provider_label
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``sidekick config``'s
...
def show_status(args):
    """Show status of all Sidekick Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Sidekick Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: sidekick auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: sidekick model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: sidekick auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: sidekick model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'sidekick-agent[vercel]')"
    ...
    try:
        from cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   sidekick gateway")
    ...
    jobs_file = get_sidekick_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_sidekick_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'sidekick doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'sidekick setup' to configure", Colors.DIM))
    print()
