...
- **DeepSeek** accepts ``deepseek-chat`` (V3), ``deepseek-reasoner``
  ...
  Sidekick revisions folded every non-reasoner input into
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Sidekick' canonical ids."""
    raw = (provider_name or "").strip().lower()
    if not raw:
        return raw
    try:
        from cli.models import normalize_provider
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Sidekick provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``sidekick_cli.models.normalize_provider()``.
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from cli.models import normalize_copilot_model_id
