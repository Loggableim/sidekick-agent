"""
Unified tool configuration for Sidekick Agent.

`sidekick tools` and `sidekick setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.sidekick/config.yaml under
...
from cli.nous_subscription import (
...
from cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _SIDEKICK_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl", "spotify", "discord", "discord_admin", "video"}

# Platform-scoped toolsets: only appear in the `sidekick tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``sidekick tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from cli.plugins import discover_plugins, get_plugin_toolsets
...
from cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. SIDEKICK_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "SIDEKICK_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "SIDEKICK_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "SIDEKICK_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "SIDEKICK_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "SIDEKICK_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``sidekick update`` and
      by ``sidekick computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `sidekick update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which("cua-driver"):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Sidekick process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_sidekick_home()} && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in sidekick-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/sidekick-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.sidekick/config.yaml")
        ...
        # Run the full `sidekick auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.sidekick/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: sidekick auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `sidekick auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: sidekick auth spotify")
        except Exception as exc:
        ...
        try:
            from cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: sidekick plugins enable observability/langfuse")
        _print_info("    Restart Sidekick for tracing to take effect.")
        _print_info("    Verify: sidekick plugins list")
    ...
    *,
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"sidekick-{platform}"
    ...
    # "sidekick-cli" (which include all _SIDEKICK_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[sidekick-cli, spotify]`` after enabling Spotify via ``sidekick
        ...
        # (e.g. "sidekick-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
    ...
    # otherwise saving via `sidekick tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"sidekick-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("sidekick-")}
    ...
    # shipped as a bundled plugin but user must opt in via `sidekick tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `sidekick tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by sidekick tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (sidekick-cli, sidekick-telegram, etc.)
    ...
    # Opening `sidekick tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(ts_key: str, config: dict = None) -> bool:
    ...
    if ts_key == "vision":
        try:
            from runtime.auxiliary_client import resolve_vision_provider_client
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `sidekick tools` gets a silent no-op
...
def _plugin_image_gen_catalog(plugin_name: str):
    ...
    """
    try:
        from runtime.image_gen_registry import get_provider
        from cli.plugins import _ensure_plugins_discovered
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `sidekick tools` and `sidekick setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Sidekick Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://sidekick-agent.sh/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from runtime._compat.shim_constants import display_sidekick_home
    print(color(f"  Tool configuration saved to {display_sidekick_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'sidekick' or gateway restart.", Colors.DIM))
    print()
