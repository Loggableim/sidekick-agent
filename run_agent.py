...
from runtime.memory_manager import StreamingContextScrubber, build_memory_context_block, sanitize_context
from runtime.think_scrubber import StreamingThinkScrubber
from runtime.retry_utils import jittered_backoff
from runtime.error_classifier import classify_api_error, FailoverReason
...
from runtime.subdirectory_hints import SubdirectoryHintTracker
from runtime.prompt_caching import apply_anthropic_cache_control
from runtime.prompt_builder import build_skills_system_prompt, build_context_files_prompt, build_environment_hints, load_soul_md, TOOL_USE_ENFORCEMENT_GUIDANCE, TOOL_USE_ENFORCEMENT_MODELS, GOOGLE_MODEL_OPERATIONAL_GUIDANCE, OPENAI_MODEL_EXECUTION_GUIDANCE
from runtime.usage_pricing import estimate_usage_cost, normalize_usage
...
from runtime.tool_guardrails import (
...
from shared.utils import atomic_json_write, base_url_host_matches, base_url_hostname, env_var_enabled, normalize_proxy_url
from cli.config import cfg_get
...
# _summarize_user_message_for_log is imported from runtime.codex_responses_adapter
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"SidekickAgent/{_SIDEKICK_VERSION}",
    }
...
class AIAgent:
    ...
    def __init__(
        ...
        Args:
            ...
            load_soul_identity (bool): If True, still use ~/.sidekick/SOUL.md as the primary
        ...
        self._cache_ttl = "5m"
        try:
            from cli.config import load_config as _load_pc_cfg
        ...
        if self.api_mode == "anthropic_messages":
            from runtime.anthropic_adapter import build_anthropic_client, resolve_anthropic_token
            # Bedrock + Claude → use AnthropicBedrock SDK for full feature parity
            # (prompt caching, thinking budgets, adaptive thinking).
            _is_bedrock_anthropic = self.provider == "bedrock"
            if _is_bedrock_anthropic:
                from runtime.anthropic_adapter import build_anthropic_bedrock_client
            ...
            try:
                from cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    ...
                    from cli.models import copilot_default_headers
                    ...
                    from runtime.auxiliary_client import _codex_cloudflare_headers
                    ...
                    if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                        ...
                        if not _fb_resolved:
                            raise RuntimeError(
                                ...
                                f"variable, or switch to a different provider with `sidekick model`."
                            )
                    if not getattr(self, "_fallback_activated", False):
                        # No provider configured — reject with a clear message.
                        raise RuntimeError(
                            "No LLM provider configured. Run `sidekick model` to "
                            "select a provider, or run `sidekick setup` for first-time "
                            "configuration."
                        )
        ...
        if not skip_memory:
            try:
                ...
                if _mem_provider_name:
                    from runtime.memory_manager import MemoryManager as _MemoryManager
        ...
        try:
            from runtime.auxiliary_client import _compression_threshold_for_model as _cthresh_fn
        ...
        try:
            from cli.config import get_compatible_custom_providers
        ...
        if _config_context_length is None and _custom_providers:
            try:
                from cli.config import get_custom_provider_context_length
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from cli.plugins import get_plugin_context_engine
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by Sidekick Agent.  Choose a model with at least "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        ...
        try:
            from runtime.model_metadata import MINIMUM_CONTEXT_LENGTH
            from cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            ...
            try:
                from cli.config import load_config, get_compatible_custom_providers
                _sm_cfg = load_config()
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``sidekick chat -q``. In that mode,
    ...
    def _emit_stream_drop(
        ...
        *,
        ...
        ``sidekick logs --level WARNING | grep "Stream drop"`` to inspect.
        """
    ...
    def _check_compression_model_feasibility(self) -> None:
        ...
        if not self.compression_enabled:
            return
        try:
            from runtime.auxiliary_client import (
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `sidekick setup` or set OPENROUTER_API_KEY."
                )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Sidekick "
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        AIAgent.  Sidekick never re-renders parts of this string mid-
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        one cached block — Sidekick never rebuilds or reinjects parts of it
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        if self.provider == "copilot-acp" or str(client_kwargs.get("base_url", "")).startswith("acp://copilot"):
            from runtime.copilot_acp_client import CopilotACPClient
        ...
        if self.provider == "gemini":
            from runtime.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from cli.copilot_auth import resolve_copilot_token
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        ...
        try:
            from runtime.anthropic_adapter import resolve_anthropic_token, build_anthropic_client
        ...
        from runtime.anthropic_adapter import _is_oauth_token
        self._is_anthropic_oauth = _is_oauth_token(new_token) if self.provider == "anthropic" else False
        return True

    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        from runtime.auxiliary_client import _AI_GATEWAY_HEADERS, build_or_headers
    ...
    def _swap_credential(self, entry) -> None:
        ...
        if self.api_mode == "anthropic_messages":
            from runtime.anthropic_adapter import build_anthropic_client, _is_oauth_token
    ...
    def _rebuild_anthropic_client(self) -> None:
        ...
        if getattr(self, "provider", None) == "bedrock":
            ...
            from runtime.anthropic_adapter import build_anthropic_client
            self._anthropic_client = build_anthropic_client(
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        try:
            ...
            try:
                from cli.model_normalize import normalize_model_for_provider
            ...
            if fb_api_mode == "anthropic_messages":
                ...
                from runtime.anthropic_adapter import build_anthropic_client, resolve_anthropic_token, _is_oauth_token
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        try:
            from runtime.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from cli.models import lmstudio_model_reasoning_options
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            summary_extra_body = {}
            try:
                from runtime.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE as _OMIT_TEMP
            except Exception:
    ...
    def run_conversation(
        ...
        try:
            from runtime.auxiliary_client import set_runtime_main
            set_runtime_main(
        ...
        # ``sidekick logs --session <id>`` can filter a single conversation.
        from runtime._compat.shim_logging import set_session_context
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for Sidekick internals.
            #
            # Sidekick invariant: the system prompt is built ONCE per session
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if (
                        ...
                        from runtime._compat.shim_constants import display_sidekick_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Sidekick-managed OAuth/setup tokens")
