"""Shell completion script generation for sidekick CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_sidekick_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Sidekick Agent bash completion
# Add to ~/.bashrc:
#   eval "$(sidekick completion bash)"
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef sidekick
# Sidekick Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(sidekick completion zsh)"
...
_sidekick() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_sidekick_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'sidekick command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Sidekick Agent fish completion",
        "# Add to your config:",
        "#   sidekick completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __sidekick_profiles",
        "    echo default",
        "    if test -d $HOME/.sidekick/profiles",
        "        ls $HOME/.sidekick/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c sidekick -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c sidekick -f -s p -l profile"
        " -d 'Profile name' -xa '(__sidekick_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__sidekick_profiles)' -d 'Profile name'"
                )
