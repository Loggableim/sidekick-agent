"""Anthropic Messages API adapter for Sidekick Agent.

Translates between Sidekick's internal OpenAI-style message format and
...
from shared.utils import base_url_host_matches, normalize_proxy_env_vars
...
# Sidekick effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Sidekick historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Sidekick)
    ...
    # 1. Sidekick-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Sidekick configs.
...
# ── Sidekick-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.sidekick/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_SIDEKICK_OAUTH_FILE = get_sidekick_home() / ".anthropic_oauth.json"
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print()
    print("Authorize Sidekick with your Claude Pro/Max subscription.")
    print()
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if _SIDEKICK_OAUTH_FILE.exists():
        try:
            data = json.loads(_SIDEKICK_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Sidekick OAuth credentials: %s", e)
    return None
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Sidekick Agent", "Claude Code")
                text = text.replace("Sidekick agent", "Claude Code")
                text = text.replace("Sidekick Agent", "Claude Code")
                text = text.replace("Sidekick agent", "Claude Code")
                text = text.replace("sidekick-agent", "claude-code")
    ...
    # silently hides reasoning text that Sidekick surfaces in its CLI. We
