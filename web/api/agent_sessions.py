...
def normalize_agent_session_source(raw_source: str | None) -> dict:
    """Return stable source metadata for Nova session rows.
...
def read_importable_agent_session_rows(
    ...
    Nova can create rows in ``state.db.sessions`` before a session has
    ...
    sidebar. This mirrors Nova CLI's session-list behaviour: interactive
    ...
    with closing(sqlite3.connect(str(db_path), timeout=1.0)) as conn:
        ...
        # Older Nova versions may not have source tracking. Without a
...
def read_session_lineage_metadata(db_path: Path, session_ids: list[str] | set[str]) -> dict[str, dict]:
    ...
    WebUI sessions are persisted as JSON files, but Nova also mirrors
