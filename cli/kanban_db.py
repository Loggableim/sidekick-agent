...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``sidekick kanban boards
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``sidekick kanban boards switch <slug>``
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``sidekick kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``sidekick kanban log`` unambiguous even when multiple
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``sidekick kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``sidekick --skills ...`` alongside the built-in
    ...
    # `sidekick --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
    ...
    for attempt in range(2):
        ...
        try:
            with write_txn(conn):
                # This lookup must live under the same write lock as the
                # insert.  Otherwise two Sidekick processes can both observe
                # a missing key and create separate tasks before either one
                # commits.
                if idempotency_key:
                    row = conn.execute(
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `sidekick kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``sidekick kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``sidekick kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``sidekick kanban complete <id>``) works without requiring
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``sidekick kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``sidekick -p <profile> chat -q ...`` subprocess.
    ...
    # `sidekick kanban log` on a specific board reads its own file and
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``sidekick kanban daemon`` is systemd-friendly.
...
def task_age(task: Task) -> dict:
    ...
    time_to_complete = completed - (started or created) if completed else None
    return {
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``sidekick kanban assignees`` for the terminal.
