...
~/.sidekick/skills/. Existing skills (bundled, hub-installed, or user-created)
...
from shared.utils import atomic_replace, is_truthy_value
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `sidekick config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.sidekick/skills/ (single source of truth)
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    ``_find_skill`` first).
    """
    from runtime.skill_utils import get_all_skills_dirs
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`sidekick curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.sidekick/skills/) first, then any
    external dirs configured via skills.external_dirs.  Returns
    {"path": Path} or None.
    """
    from runtime.skill_utils import EXCLUDED_SKILL_DIRS, get_all_skills_dirs
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_sidekick_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `sidekick curator unpin <name>`. "
