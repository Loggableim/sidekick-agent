"""
Multi-provider authentication system for Sidekick Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.sidekick/auth.json with cross-process file locking.
...
DEFAULT_NOUS_CLIENT_ID = "sidekick-cli"
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `sidekick model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.sidekick/auth.json
# =============================================================================
...
def _auth_file_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".sidekick" / "auth.json").resolve(strict=False)
        try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".sidekick" / "auth.json"
            try:
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Sidekick.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `sidekick logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'sidekick doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'sidekick model' for available providers, or run 'sidekick doctor' to diagnose config issues."
    ...
    try:
        from runtime.bedrock_adapter import has_aws_credentials
    ...
    raise AuthError(
        "No inference provider configured. Run 'sidekick model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.sidekick/.env.",
...
#
# Tokens live in ~/.sidekick/auth/google_oauth.json (managed by agent.google_oauth).
...
def _spotify_client_id(
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set SIDEKICK_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `sidekick auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `sidekick auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `sidekick auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `sidekick auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.sidekick/.env, and return it.
    ...
    print("       App name:     anything (e.g. sidekick-agent)")
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize Sidekick:")
    print(authorize_url)
    print()
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.sidekick/auth.json (not ~/.codex/)
#
# Sidekick maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Sidekick auth store (~/.sidekick/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `sidekick auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `sidekick auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `sidekick auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `sidekick auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Sidekick auth store (~/.sidekick/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Sidekick auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `sidekick auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Sidekick auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Sidekick's own Codex token store."""
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other Sidekick processes
...
# so a new `sidekick --profile <name> auth add nous --type oauth` can one-tap
...
# ``get_default_hermes_root()`` returns — ``~/.sidekick`` on Linux/macOS,
# ``%LOCALAPPDATA%\\sidekick`` on native Windows, or the Docker/custom root.
...
def _nous_shared_auth_dir() -> Path:
    ...
    Linux/macOS classic installs land at ``~/.sidekick/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\sidekick\\shared\\``, and
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's Sidekick root, refuse rather than silently corrupt cross-profile
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Sidekick's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Sidekick's next refresh uses it, and the whole
    ...
    if "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another Sidekick install sharing "
            "~/.sidekick/auth.json) called POST /api/oauth/token with Sidekick's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Sidekick may call the "
            "refresh endpoint. For health checks, use `sidekick auth status` "
            "instead.\n"
            "Re-authenticate with: sidekick auth add nous"
        )
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Sidekick is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("SIDEKICK_PORTAL_BASE_URL") or os.getenv("HERMES_PORTAL_BASE_URL")
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "sidekick-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``sidekick auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``sidekick auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `sidekick auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Sidekick is not logged into Nous Portal.",
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `sidekick auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `sidekick auth` and
    # `sidekick model` store device_code tokens.
    try:
        from runtime.credential_pool import load_pool
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `sidekick logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'sidekick model' or 'sidekick setup' instead."""
    print("The 'sidekick login' command has been removed.")
    print("Use 'sidekick auth' to manage credentials,")
    print("'sidekick model' to select a provider, or 'sidekick setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.sidekick/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Sidekick-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Sidekick auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Sidekick will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("Sidekick will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Sidekick gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Sidekick creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Sidekick auth store
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Sidekick auth store (~/.sidekick/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Sidekick login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `sidekick model` and select "
            "MiniMax (OAuth).",
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Sidekick login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `sidekick model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Sidekick builds still see
        ...
        if not selected_model:
            ...
            print("  Run `sidekick model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Sidekick will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `sidekick model` or configure an API key to use Sidekick.")
        else:
