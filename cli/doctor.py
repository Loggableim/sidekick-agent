"""
Doctor command for sidekick CLI.

Diagnoses issues with Sidekick Agent setup.
"""
...
from runtime._compat.shim_constants import is_termux as _is_termux
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    print(color("│                 🩺 Sidekick Doctor                      │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if env_path.exists():
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'sidekick setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'sidekick setup' to create one")
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'sidekick config set model.provider <valid_provider>'"
                    )
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'sidekick setup' to migrate config")
                else:
                    issues.append("Run 'sidekick doctor --fix' or 'sidekick setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'sidekick doctor --fix'")
        except Exception:
            pass
    ...
    if sidekick_home.exists():
        ...
        sidekick_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = sidekick_home / subdir_name
    ...
    if soul_path.exists():
        ...
        if should_fix:
            ...
            soul_path.write_text(
                ...
                "<!-- Edit this file to customize how Sidekick communicates. -->\n\n"
    ...
    memories_dir = sidekick_home / "memories"
    ...
    state_db_path = sidekick_home / "state.db"
    ...
    wal_path = sidekick_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    # =========================================================================
    # Check: Command installation (sidekick bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "sidekick"
        ...
        _cmd_link = _cmd_link_dir / "sidekick"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(sidekick not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/sidekick → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/sidekick points to wrong target",
                ...
                check_fail(
                    f"{_cmd_link_display}/sidekick not found",
                    "(sidekick command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/sidekick → {_venv_bin}")
                    fixed_count += 1
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'sidekick-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'sidekick-agent[vercel]'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `sidekick doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'sidekick config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            if base and base.rstrip("/").endswith("/anthropic"):
                from runtime.auxiliary_client import _to_openai_base_url
    ...
    # never the legitimate source for `sidekick doctor`.
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: sidekick memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: sidekick memory setup")
                issues.append("No Honcho API key — run 'sidekick memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run sidekick memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: sidekick memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: sidekick memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "sidekick -p" in content:
                            _m = _re.search(r"sidekick -p (\S+)", content)
