...
def _normalize_provider(provider: str) -> str:
    ...
    # Built-in canonical provider ids must win over same-named custom providers.
    # Runtime routing has the same guard; without it, a config entry like
    # providers.opencode-go makes `sidekick auth reset opencode-go` target
    # custom:opencode-go instead of the built-in opencode-go pool.
...
def _provider_base_url(provider: str) -> str:
    ...
    if provider.startswith(CUSTOM_POOL_PREFIX):
        from runtime.credential_pool import _get_custom_provider_config
...
def auth_add_command(args) -> None:
    ...
    if not requested_type:
        if provider.startswith(CUSTOM_POOL_PREFIX):
            ...
            requested_type = AUTH_TYPE_OAUTH if provider in {"anthropic", "openai-codex", "qwen-oauth", "google-gemini-cli", "minimax-oauth"} else AUTH_TYPE_API_KEY
    ...
    if provider == "anthropic":
        from runtime import anthropic_adapter as anthropic_mod

        creds = anthropic_mod.run_sidekick_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:sidekick_pkce",
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `sidekick auth
    ...
    if provider == "google-gemini-cli":
        from runtime.google_oauth import run_gemini_oauth_login_pure
    ...
    raise SystemExit(f"`sidekick auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Sidekick reads from
    ...
    from runtime.credential_sources import find_removal_step
    from cli.auth import suppress_credential_source
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `sidekick auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `sidekick auth` is called bare."""
    ...
    try:
        from runtime.bedrock_adapter import has_aws_credentials, resolve_aws_auth_env_var, resolve_bedrock_region
