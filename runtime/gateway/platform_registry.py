...
Usage (plugin side)::
    ...
    from runtime.gateway.platform_registry import platform_registry, PlatformEntry
...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``sidekick setup`` display).
    ...
    # built-ins).  Used by ``sidekick gateway setup`` to auto-enable the
