...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    sidekick_node_bin = str(sidekick_home / "node" / "bin")
    return [sidekick_node_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Sidekick silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Sidekick CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-sidekick_…`` the resulting socket path exceeds the
...
def _reap_orphaned_browser_sessions():
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-sidekick_*"))
...
BROWSER_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "description": "Run a versioned browser action sequence against the visible Sidekick WebUI browser. Each step can be navigate/open, click, type, scroll, wait, press, back, forward, reload, or stop. Use this when you need several actions in one round-trip and want the user to see the same live browser updating step by step.",
        "parameters": {
            "type": "object",
            "properties": {
                "version": {
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Sidekick-managed
    ...
    # Build an extended search PATH including Sidekick-managed Node, macOS
...
def browser_navigate(url: str, task_id: Optional[str] = None) -> str:
    ...
    """
    import urllib.parse
    from runtime.redact import _PREFIX_RE
...
def browser_action_v1(steps: List[Dict[str, Any]], task_id: Optional[str] = None) -> str:
    """Run a versioned browser action sequence in the visible Sidekick WebUI browser."""
    if _is_camofox_mode():
        return json.dumps({
            "success": False,
    ...
    return json.dumps({
        "success": False,
        "error": "WebUI browser action sequence requires the Sidekick WebUI browser bridge",
    }, ensure_ascii=False)
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = sidekick_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    screenshots_dir = get_sidekick_dir("cache/screenshots", "browser_screenshots")
...
def _chromium_search_roots() -> List[str]:
    ...
    1. ``PLAYWRIGHT_BROWSERS_PATH`` when set (Docker image sets this to
       ``/opt/sidekick/.playwright`` — legacy Docker path).
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/sidekick-agent:latest")
                else:
