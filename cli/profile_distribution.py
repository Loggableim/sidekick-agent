...
* ``sidekick profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``sidekick skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``sidekick profile``, not a parallel tree):

    sidekick profile install <source> [--name N] [--alias] [--force] [--yes]
    sidekick profile update  <name>  [--force-config] [--yes]
    sidekick profile info    <name>
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``sidekick profile update`` can
...
def plan_install(
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.sidekick).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `sidekick profile update` to upgrade in place, "
