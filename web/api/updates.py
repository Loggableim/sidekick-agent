...
from web.api.config import REPO_ROOT, STREAMS, STREAMS_LOCK
...
def _detect_webui_version() -> str:
    ...
    out, ok = _run_git(['describe', '--tags', '--always', '--dirty'], version_root, timeout=3)
    if ok and out:
        return out
...
def _check_repo(path, name):
    ...
    _, fetch_ok = _run_git(['fetch', 'origin', '--quiet'], git_path, timeout=15)
    if not fetch_ok:
    ...
    upstream, ok = _run_git(['rev-parse', '--abbrev-ref', '@{upstream}'], git_path)
    if ok and upstream:
        # upstream is like "origin/feat/foo" — use it directly in rev-list
        compare_ref = upstream
    else:
        branch = _detect_default_branch(git_path)
        compare_ref = f'origin/{branch}'

    # Count commits behind
    out, ok = _run_git(['rev-list', '--count', f'HEAD..{compare_ref}'], git_path)
    ...
    mb_full, mb_ok = _run_git(['merge-base', 'HEAD', compare_ref], git_path)
    if mb_ok and mb_full:
        short, ok = _run_git(['rev-parse', '--short', mb_full], git_path)
        current = short if (ok and short) else None
    else:
        current = None
    latest, _ = _run_git(['rev-parse', '--short', compare_ref], git_path)

    # Get repo URL for "What's new?" link
    remote_url, _ = _run_git(['remote', 'get-url', 'origin'], git_path)
