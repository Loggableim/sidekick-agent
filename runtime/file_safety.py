...
def _sidekick_home_path() -> Path:
    ...
    try:
        ...
        return Path(os.path.expanduser("~/.sidekick"))
...
def get_safe_write_root() -> Optional[str]:
    ...
    root = os.getenv("SIDEKICK_WRITE_SAFE_ROOT") or os.getenv("HERMES_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    for blocked in blocked_dirs:
        try:
            ...
            continue
        return (
            f"Access denied: {path} is an internal Sidekick cache file "
