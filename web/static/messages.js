...
// ── Session-scoped subagent overview (durable journal) ─────────────────────
let _subagentOverviewSource = null;
let _subagentOverviewSessionId = null;
let _subagentLastEventId = 0;
const _SUBAGENT_POLL_FALLBACK_MS = 5000;
function _subagentOverviewPath(sessionId){return '/api/subagents?session_id='+encodeURIComponent(String(sessionId||''))+'&status=all&limit=50';}
function _subagentStreamPath(sessionId){return '/api/subagents/events/stream?session_id='+encodeURIComponent(String(sessionId||''));}
function _subagentStatusGroup(status){const value=String(status||'').toLowerCase();if(['queued','waiting','paused'].includes(value))return 'waiting';if(value==='completed')return 'completed';if(['failed','interrupted','abandoned'].includes(value))return 'failed';return 'active';}
function _subagentOverviewTime(entry){const group=_subagentStatusGroup(entry&&entry.status);if(group==='active'||group==='waiting')return _subagentElapsed(entry&&entry.started_at)||'just now';const ended=Number(entry&&(entry.finished_at||entry.updated_at)||0);if(!ended)return 'finished';const age=Math.max(0,Math.round(Date.now()/1000-ended));return age<60?'just now':age<3600?Math.floor(age/60)+'m ago':new Date(ended*1000).toLocaleString();}
function _subagentOverviewHints(entry){const text=[entry&&entry.last_step,entry&&entry.summary].filter(Boolean).join(' ');const out=[];if(/\b(test|pytest|jest|vitest|playwright)\b/i.test(text))out.push('tests noted');if(/\b(commit|[0-9a-f]{7,40})\b/i.test(text))out.push('commit noted');return out.join(' · ');}
function _subagentOverviewDetail(label,value){if(!value)return null;const row=document.createElement('div');row.className='subagent-panel-detail-line';const key=document.createElement('span');key.textContent=label;const text=document.createElement('span');text.textContent=String(value);row.append(key,text);return row;}
function _subagentOverviewRow(entry,sessionId,onRefresh){const row=document.createElement('details');const group=_subagentStatusGroup(entry&&entry.status);row.className='subagent-panel-row subagent-panel-overview-row status-'+group;const summary=document.createElement('summary');const dot=document.createElement('span');dot.className='subagent-panel-status-dot status-'+group;const main=document.createElement('div');main.className='subagent-panel-row-main';const goal=document.createElement('div');goal.className='subagent-panel-goal';goal.textContent=String(entry&&(entry.goal_summary||entry.goal)||'Subagent');const meta=document.createElement('div');meta.className='subagent-panel-sub';meta.textContent=[entry&&entry.status,entry&&entry.role,entry&&entry.model,_subagentOverviewTime(entry)].filter(Boolean).join(' · ');main.append(goal,meta);summary.append(dot,main);row.append(summary);const detail=document.createElement('div');detail.className='subagent-panel-detail';[_subagentOverviewDetail('Last step',entry&&entry.last_step),_subagentOverviewDetail('Summary',entry&&entry.summary),_subagentOverviewDetail('Error',entry&&entry.error_reason),_subagentOverviewDetail('Parent',entry&&entry.parent_id),_subagentOverviewDetail('Session',entry&&entry.session_id),_subagentOverviewDetail('Hints',_subagentOverviewHints(entry))].forEach(line=>{if(line)detail.appendChild(line);});const timeline=document.createElement('ol');timeline.className='subagent-panel-timeline';detail.appendChild(timeline);const runId=String(entry&&entry.subagent_id||'');row.addEventListener('toggle',async()=>{if(!row.open||!runId||row.dataset.loaded==='1')return;row.dataset.loaded='1';try{const payload=await api('/api/subagents/'+encodeURIComponent(runId)+'?session_id='+encodeURIComponent(sessionId));if(String(sessionId)!==String(_subagentCurrentSessionId()))return;(Array.isArray(payload&&payload.events)?payload.events:[]).slice(-200).forEach(event=>{const item=document.createElement('li');item.textContent=[event&&event.kind,event&&event.detail].filter(Boolean).join(' · ');timeline.appendChild(item);});if(!timeline.childElementCount)timeline.textContent='No recorded events yet.';}catch(_){row.dataset.loaded='';timeline.textContent='Timeline is currently unavailable.';}});if(group==='active'&&runId){const interrupt=document.createElement('button');interrupt.type='button';interrupt.className='subagent-panel-row-btn danger';interrupt.textContent='Interrupt';interrupt.addEventListener('click',async event=>{event.preventDefault();event.stopPropagation();try{await api('/api/subagents',{method:'POST',body:JSON.stringify({subagent_id:runId})});if(typeof onRefresh==='function')onRefresh();}catch(error){showToast('Subagent interrupt failed: '+(error&&error.message?error.message:runId));}});detail.appendChild(interrupt);}row.appendChild(detail);return row;}
function _subagentRenderSection(container,payload,sessionId,onRefresh){if(!container)return;container.innerHTML='';const groups={active:[],waiting:[],completed:[],failed:[]};(Array.isArray(payload&&payload.runs)?payload.runs:[]).forEach(entry=>groups[_subagentStatusGroup(entry&&entry.status)].push(entry));if(!Object.values(groups).some(items=>items.length)){const empty=document.createElement('div');empty.className='subagent-panel-empty';empty.textContent='No recorded subagent activity in this chat yet.';container.appendChild(empty);return;}[['active','Active'],['waiting','Waiting / paused'],['completed','Completed'],['failed','Failed']].forEach(([key,label])=>{const section=document.createElement('section');section.className='subagent-panel-section';const heading=document.createElement('h4');heading.textContent=label+(groups[key].length?' · '+groups[key].length:'');section.appendChild(heading);if(groups[key].length)groups[key].sort((a,b)=>Number(b.updated_at||0)-Number(a.updated_at||0)).forEach(entry=>section.appendChild(_subagentOverviewRow(entry,sessionId,onRefresh)));else{const empty=document.createElement('div');empty.className='subagent-panel-section-empty';empty.textContent='None';section.appendChild(empty);}container.appendChild(section);});}
function _subagentSetOffline(container){if(!container)return;container.innerHTML='';const offline=document.createElement('div');offline.className='subagent-panel-offline';offline.textContent='Subagent history is offline. Retrying with bounded polling.';container.appendChild(offline);}
window.renderSubagentOverviewInto=_subagentRenderSection;
function _subagentRenderPanel(payload){const sid=_subagentCurrentSessionId();if(!sid){stopSubagentPolling();return;}const state=_subagentEnsurePanelState();state.visible=true;state.sessionId=sid;state.spawnPaused=!!(payload&&payload.spawn_paused);state.status.className='subagent-panel-status'+(state.spawnPaused?' paused':'');const count=Array.isArray(payload&&payload.runs)?payload.runs.length:0;state.status.textContent=count?(count+' recorded'):(state.spawnPaused?'spawning paused':'Subagents');state.toggleBtn.textContent=state.spawnPaused?'Resume spawning':'Pause spawning';_subagentRenderSection(state.list,payload,sid,()=>void _refreshSubagentPanel());if(!state.element.parentNode){const transcript=document.getElementById('messages')||document.querySelector('.messages');if(transcript)transcript.appendChild(state.element);}}
async function _refreshSubagentPanel(){const sid=_subagentCurrentSessionId();if(!sid){stopSubagentPolling();return;}try{const data=await api(_subagentOverviewPath(sid));if(sid!==_subagentCurrentSessionId())return;_subagentRenderPanel(data||{});}catch(_){if(_subagentPanelState&&_subagentPanelState.list)_subagentSetOffline(_subagentPanelState.list);}}
function _subagentStartStream(sid){if(typeof EventSource!=='function'||!sid)return;if(_subagentOverviewSource&&_subagentOverviewSessionId===sid)return;if(_subagentOverviewSource)try{_subagentOverviewSource.close();}catch(_){};_subagentOverviewSessionId=sid;_subagentOverviewSource=new EventSource(_subagentStreamPath(sid));const refresh=event=>{if(sid!==_subagentCurrentSessionId())return;const id=Number(event&&event.lastEventId||0);if(id&&id<=_subagentLastEventId)return;if(id)_subagentLastEventId=id;void _refreshSubagentPanel();};_subagentOverviewSource.addEventListener('events',refresh);_subagentOverviewSource.addEventListener('snapshot',refresh);_subagentOverviewSource.addEventListener('hello',refresh);_subagentOverviewSource.onerror=()=>{if(sid===_subagentCurrentSessionId())void _refreshSubagentPanel();};}
function _ensureSubagentPolling(){const sid=_subagentCurrentSessionId();if(!sid){stopSubagentPolling();return;}if(_subagentPollSessionId!==sid||!_subagentPollTimer)startSubagentPolling(sid);}
function startSubagentPolling(sid){stopSubagentPolling();if(!sid)return;_subagentPollSessionId=sid;_subagentLastEventId=0;_subagentPollTimer=setInterval(()=>{if(_subagentCurrentSessionId()!==sid){stopSubagentPolling();return;}void _refreshSubagentPanel();},_SUBAGENT_POLL_FALLBACK_MS);_subagentStartStream(sid);void _refreshSubagentPanel();}
function stopSubagentPolling(){if(_subagentPollTimer){clearInterval(_subagentPollTimer);_subagentPollTimer=null;}if(_subagentOverviewSource)try{_subagentOverviewSource.close();}catch(_){};_subagentOverviewSource=null;_subagentOverviewSessionId=null;_subagentPollSessionId=null;}
