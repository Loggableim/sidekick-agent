"""
Sidekick -- Workspace and file system helpers.
...
def _workspace_access_error(candidate: Path, *, missing_label: str = "Path does not exist") -> str | None:
    ...
    try:
        ...
        return (
            ...
            f"Folders permission to the Sidekick app or server process, then try again."
        )
...
def _trusted_space_project_roots() -> list[Path]:
    """Return configured Space project directories that are safe workspace roots."""
    roots: list[Path] = []
    try:
    ...
    for space in spaces:
        try:
            ...
            if not root.is_dir():
                continue
            if _is_blocked_workspace_path(root, project_dir):
                continue
...
def resolve_trusted_workspace(path: str | Path | None = None) -> Path:
    ...
    # (D) Trusted if it is under the Spaces root directory (SPACES_ROOT).
    #     Space directories are managed by the app and are inherently
    #     safe without requiring the user to add them to the workspace list.
    try:
        from web.api.space_engine import SPACES_ROOT as _spaces_root
        candidate.relative_to(_spaces_root)
        return candidate
    except (ImportError, ValueError):
        pass

    # (E) Trusted if it is equal to or under a configured Space project_dir.
    #     Space switching already treats project_dir as the active workspace;
    #     trusting it here keeps /api/session/update, /api/chat/start and file
    #     APIs aligned without forcing duplicate entries in workspaces.json.
    for project_root in _trusted_space_project_roots():
