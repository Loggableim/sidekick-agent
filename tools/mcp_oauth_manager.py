...
`mcp_tool.py`, and `sidekick_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# SidekickMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_sidekick_provider_class() -> Optional[type]:
    ...
    class SidekickMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._sidekick_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`SidekickTokenStorage` persisting an absolute
            ...
            from tools.mcp_oauth import SidekickTokenStorage
            if (
                isinstance(storage, SidekickTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._sidekick_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._sidekick_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import SidekickTokenStorage
                        if isinstance(storage, SidekickTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._sidekick_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import SidekickTokenStorage
            if not isinstance(storage, SidekickTokenStorage):
                return
    ...
    return SidekickMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_SIDEKICK_PROVIDER_CLS: Optional[type] = _make_sidekick_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`SidekickMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _SIDEKICK_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = SidekickTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``sidekick mcp remove <name>`` and (indirectly) by
        ``sidekick mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
