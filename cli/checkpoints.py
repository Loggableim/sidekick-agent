"""`sidekick checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.sidekick/checkpoints/``.  Actions:

    sidekick checkpoints               # same as `status`
    sidekick checkpoints status        # total size, project count, breakdown
    sidekick checkpoints list          # per-project checkpoint counts + workdir
    sidekick checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    sidekick checkpoints clear [-f]    # nuke the entire base (asks first)
    sidekick checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    sidekick checkpoints
    sidekick checkpoints prune --retention-days 3 --max-size-mb 200
    sidekick checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: sidekick checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``sidekick checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `sidekick checkpoints` → status
