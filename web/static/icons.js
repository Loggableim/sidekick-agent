...
//
// ── Iconify integration (self-hosted SVGs) ─────────────────────────────────
// ic(name, size) — local SVG files: skill-icons/ primary, devicon/ fallback,
//                  li() as last resort (Lucide).
// eji(name, size) — fluent-emoji-flat local SVGs.
...
// ── Local self-hosted icon sets ───────────────────────────────────────────
// Icons downloaded to static/icons/{si,di,ph,fe}/
// ic(name) → skill-icons (si/) → devicon (di/) → li() (Lucide)

var _SI = {
  terminal:'bash-light', write_file:'vscode-light', execute_code:'python-light',
  patch:'vim-light', web_search:'google-light', web_extract:'google-light',
  browser_navigate:'chrome-light', git_branch:'git', docker:'docker',
  python:'python-light', javascript:'javascript', typescript:'typescript',
  html:'html', css:'css', react:'react-light', nodejs:'nodejs-light',
  rust:'rust', golang:'golang', java:'java-light', github:'github-light',
  vscode:'vscode-light', figma:'figma-light', kubernetes:'kubernetes',
  linux:'linux-light', bash:'bash-light', tensorflow:'tensorflow-light',
  pytorch:'pytorch-light',
};

var _DI = {
  terminal:'bash', write_file:'vscode', execute_code:'python',
  web_search:'google', web_extract:'google', browser_navigate:'chrome',
  patch:'vim', git:'git', github:'github', nodejs:'nodejs', python:'python',
  react:'react', typescript:'typescript', javascript:'javascript', rust:'rust',
  go:'go', java:'java', docker:'docker', kubernetes:'kubernetes', linux:'linux',
  bash:'bash', vscode:'vscode', figma:'figma', vue:'vuejs', svelte:'svelte',
  angular:'angular', mongodb:'mongodb', postgresql:'postgresql', redis:'redis',
...
function ic(name, size) {
  ...
  if (name && name.indexOf(':') > 0) {
    // Raw iconify name like "skill-icons:python" or "ph:gear"
    var parts = name.split(':');
    var prefix = parts[0], icon = parts.slice(1).join(':');
    if (prefix === 'skill-icons') file = 'si/' + icon;
    else if (prefix === 'devicon') file = 'di/' + icon;
    else if (prefix === 'ph') file = 'ph/' + icon;
    else if (prefix === 'fluent-emoji-flat') file = 'fe/' + icon;
    else file = prefix + '/' + icon;
  } else {
    file = _SI[name] ? ('si/' + _SI[name]) : (_DI[name] ? ('di/' + _DI[name]) : null);
  }
  if (!file) return li(name, size);
  return '<img src="static/icons/' + file + '.svg" width="' + size + '" height="' + size + '" alt="" style="display:inline-block;vertical-align:-0.15em;flex-shrink:0">';
}

function eji(name, size) {
  size = size || 16;
  var file;
  if (name && name.indexOf(':') > 0) {
    var parts = name.split(':');
    if (parts[0] === 'fluent-emoji-flat') file = 'fe/' + parts.slice(1).join(':');
