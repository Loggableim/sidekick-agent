"""
Gateway subcommand for sidekick CLI.

Handles: sidekick gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "sidekick CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``sidekick gateway status`` from falsely counting the ``sidekick`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``sidekick gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "sidekick_cli.main gateway",
        "sidekick_cli.main --profile",
        "sidekick_cli.main -p",
        "sidekick_cli/main.py gateway",
        "sidekick_cli/main.py --profile",
        "sidekick_cli/main.py -p",
        "sidekick gateway",
        "gateway/run.py",
    ]
    current_home = str(get_sidekick_home().resolve())
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``sidekick update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Sidekick profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Sidekick profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "sidekick_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``sidekick update`` users
    # with no running gateway until they re-invoke ``sidekick gateway``
    ...
    from cli._subprocess_compat import windows_detach_popen_kwargs
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}sidekick gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}sidekick gateway restart{scope_flag}")
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `sidekick gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``sidekick gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
from runtime._compat.shim_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "sidekick-gateway"
SERVICE_DESCRIPTION = "Sidekick Agent Gateway - Messaging Platform Integration"
...
# Legacy service names from older Sidekick installs that predate the
...
def print_legacy_unit_warning() -> None:
    """Warn about legacy Sidekick gateway unit files if any are installed.
...
def remove_legacy_hermes_units(
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: sidekick gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo sidekick gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    sidekick gateway uninstall")
    print_info("    sudo sidekick gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Sidekick can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo sidekick gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo sidekick gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo sidekick gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Sidekick often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.sidekick`` → ``ai.sidekick.gateway.plist`` (backward compatible).
    Profile ``~/.sidekick/profiles/coder`` → ``ai.sidekick.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.sidekick.gateway-{suffix}" if suffix else "ai.sidekick.gateway"
...
User={username}
Group={group_name}
ExecStart={python_path} -m sidekick_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Sidekick install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo sidekick gateway uninstall --system")
    print_info("         sidekick gateway install")
    print_info("         sidekick gateway start")
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}sidekick gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}sidekick gateway status{scope_flag}             # Check status")
...
def systemd_stop(system: bool = False):
    ...
    _sync_sidekick_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `sidekick gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `sidekick gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}sidekick gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}sidekick gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}sidekick gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}sidekick gateway start{scope_flag}")
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>sidekick_cli.main</string>",
    ]
    if profile_arg:
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Sidekick install")
    return True
...
def launchd_stop():
    ...
    # `sidekick gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Sidekick install")
    else:
        print("⚠ Service definition is stale relative to the current Sidekick install")
        print("  Run: sidekick gateway start")
    ...
    if deep:
        log_file = get_sidekick_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("SIDEKICK_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Sidekick gateway as root inside the official Docker image."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # /restart) rather than through `sidekick gateway restart` which already
    ...
    # `sidekick gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Sidekick Gateway Starting...                │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if (os.environ.get("SIDEKICK_GATEWAY_EXIT_DIAG")) != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Sidekick will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @sidekick:matrix.example.org"},
    ...
    },
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Sidekick delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Sidekick agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Sidekick will use (e.g., sidekick@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Sidekick connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: sidekick pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Sidekick will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``sidekick setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.sidekick/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_sidekick_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'sidekick pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: sidekick pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'sidekick gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Sidekick will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Sidekick will store the returned account_id/token in ~/.sidekick/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `sidekick gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_sidekick_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `sidekick pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `sidekick pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"SidekickAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``sidekick_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.sidekick/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.sidekick/.env: {', '.join(required)}")
    else:
        ...
        if service_running:
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: sidekick gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  sidekick gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: sidekick gateway run")
                print_info("  For persistence:   tmux new -s sidekick 'sidekick gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from runtime._compat.shim_constants import display_sidekick_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: sidekick gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup sidekick gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: sidekick gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'sidekick gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``sidekick gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: sidekick gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: sidekick gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s sidekick 'sidekick gateway run'")
                print()
            ...
            print()
            print("  sidekick gateway run                              # direct foreground")
            print("  tmux new -s sidekick 'sidekick gateway run'         # persistent via tmux")
            print("  nohup sidekick gateway run > ~/.sidekick/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: sidekick gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: sidekick gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: sidekick gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: sidekick gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    sidekick gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: sidekick gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  sidekick gateway install")
                else:
                    print("To install as a service:")
                    print("  sidekick gateway install")
                    print("  sudo sidekick gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  sidekick gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup sidekick gateway run > ~/.sidekick/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s sidekick 'sidekick gateway run'         # persistent via tmux")
                    print("  nohup sidekick gateway run > ~/.sidekick/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  sidekick gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  sidekick gateway install  # Install as user service")
                    print("  sudo sidekick gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Sidekick gateway unit files from
