...
def register_from_config(
    ...
    *,
    ...
    from sidekick_cli.plugins import get_plugin_manager
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``sidekick hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from sidekick_cli.plugins import VALID_HOOKS
...
def allowlist_path() -> Path:
    ...
    return get_sidekick_home() / ALLOWLIST_FILENAME
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Sidekick is about to register a shell hook that will run a\n"
...
# ---------------------------------------------------------------------------
# Introspection (used by `sidekick hooks` CLI)
# ---------------------------------------------------------------------------
