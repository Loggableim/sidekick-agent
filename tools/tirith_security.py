...
it is automatically downloaded from GitHub releases to $SIDEKICK_HOME/bin/tirith.
...
def _get_sidekick_home() -> str:
    """Return the Sidekick home directory, respecting SIDEKICK_HOME env var."""
    return str(get_sidekick_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_sidekick_home(), ".tirith-install-failed")
...
def _sidekick_bin_dir() -> str:
    """Return $SIDEKICK_HOME/bin, creating it if needed."""
    d = os.path.join(_get_sidekick_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SIDEKICK_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_sidekick_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SIDEKICK_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SIDEKICK_HOME/bin/tirith
    ...
    sidekick_bin = os.path.join(_sidekick_bin_dir(), "tirith")
    if os.path.isfile(sidekick_bin) and os.access(sidekick_bin, os.X_OK):
        _resolved_path = sidekick_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return sidekick_bin
