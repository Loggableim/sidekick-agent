"""sidekick claw — OpenClaw migration commands.

Usage:
    sidekick claw migrate              # Preview then migrate (always shows preview first)
    sidekick claw migrate --dry-run    # Preview only, no changes
    sidekick claw migrate --yes        # Skip confirmation prompt
    sidekick claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    sidekick claw migrate --no-backup  # Skip pre-migration snapshot
    sidekick claw cleanup              # Archive leftover OpenClaw directories
    sidekick claw cleanup --dry-run    # Preview what would be archived
"""
...
from cli.config import get_sidekick_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "Sidekick may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Sidekick gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Sidekick gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'sidekick stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_sidekick.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_sidekick", script_path)
...
def claw_command(args):
    """Route sidekick claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: sidekick claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Sidekick")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'sidekick claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Sidekick migration."""
    ...
    # migrate-sidekick posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: sidekick claw migrate --source /path/to/.openclaw")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.sidekick/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: sidekick claw migrate --yes")
            return
    ...
    # restorable with `sidekick import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: sidekick import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Sidekick home."
            )
    ...
    # 'sidekick claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: sidekick claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: sidekick claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  sidekick claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  sidekick claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  sidekick config set OPENROUTER_API_KEY sk-or-v1-...")
