...
Usage:
    sidekick                     # Interactive chat (default)
    sidekick chat                # Interactive chat
    sidekick gateway             # Run gateway in foreground
    sidekick gateway start       # Start gateway as service
    sidekick gateway stop        # Stop gateway service
    sidekick gateway status      # Show gateway status
    sidekick gateway install     # Install gateway service
    sidekick gateway uninstall   # Uninstall gateway service
    sidekick setup               # Interactive setup wizard
    sidekick logout              # Clear stored authentication
    sidekick status              # Show status of all components
    sidekick cron                # Manage cron jobs
    sidekick cron list           # List cron jobs
    sidekick cron status         # Check if cron scheduler is running
    sidekick doctor              # Check configuration and dependencies
    sidekick honcho setup                    # Configure Honcho AI memory integration
    sidekick honcho status                   # Show Honcho config and connection status
    sidekick honcho sessions                 # List directory → session name mappings
    sidekick honcho map <name>               # Map current directory to a session name
    sidekick honcho peer                     # Show peer names and dialectic settings
    sidekick honcho peer --user NAME         # Set user peer name
    sidekick honcho peer --ai NAME           # Set AI peer name
    sidekick honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    sidekick honcho mode                     # Show current memory mode
    sidekick honcho mode [hybrid|honcho|local]  # Set memory mode
    sidekick honcho tokens                   # Show token budget settings
    sidekick honcho tokens --context N       # Set session.context() token cap
    ...
    sidekick acp                 Run as an ACP server for editor integration
    sidekick sessions browse     Interactive session picker with search
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (sidekick tools, sidekick setup, sidekick model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'sidekick {command_name}' requires an interactive terminal.\n"
...
def _apply_profile_override() -> None:
    ...
    # Mirrors sidekick_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # See issue #22502.
    sidekick_home_env = os.environ.get("SIDEKICK_HOME")
...
# Initialize centralized file logging early — all `sidekick` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from runtime._compat.shim_logging import setup_logging as _setup_logging
...
try:
    from cli.config import load_config as _load_config_early
    from runtime._compat.shim_constants import apply_ipv4_preference as _apply_ipv4
...
def _has_any_provider_configured() -> bool:
    ...
    auth_file = get_sidekick_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo sidekick {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo sidekick {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  sidekick --tui --resume {target}")
    if title:
        print(f'  sidekick --tui -c "{title}"')
    print()
...
def _ensure_tui_node() -> None:
    ...
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SIDEKICK_SKIP_NODE_BOOTSTRAP"):
        return
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("SIDEKICK_NODE")
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not (os.environ.get("SIDEKICK_QUIET")):
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from cli.oneshot import _normalize_toolsets
...
def _pin_kanban_board_env() -> None:
    ...
    ``sidekick kanban boards switch`` from another session can flip the file
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("SIDEKICK_TUI") == "1" == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'sidekick sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'sidekick setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from cli.banner import prefetch_update_check
...
def cmd_gateway(args):
    ...
    from cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Sidekick?")
        print()
    ...
    session_dir = get_sidekick_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: sidekick gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  sidekick gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Sidekick Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  sidekick gateway")
        ...
        print()
        print("  Or install as a service: sidekick gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'sidekick whatsapp' to try again.")
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``sidekick model``) and the setup wizard
    ...
    from cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("SIDEKICK_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'sidekick model' for "
                "available providers, or run 'sidekick doctor' to diagnose config "
                "issues."
            )
    ...
    from cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.sidekick/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.sidekick/.env if the active provider is not 'custom'.
    ...
    """
    from cli.config import get_env_value, save_env_value, load_config
...
# `sidekick model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `sidekick model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Sidekick only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``sidekick model`` flow, then route aux tasks to them here.
    """
    from cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from cli.auth import _prompt_model_selection
    from cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    _codex_token = None
    # Prefer credential pool (where `sidekick auth` stores device_code tokens),
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from cli.models import _PROVIDER_MODELS
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.sidekick/config.yaml.
    """
    from cli.auth import (
    ...
    if not status.get("logged_in"):
        try:
            from runtime.google_oauth import resolve_project_id_from_env, start_oauth_flow
...
def _model_flow_custom(config):
    ...
    """
    from cli.auth import _save_model_choice, deactivate_provider
    from cli.config import get_env_value, load_config, save_config
    ...
    from cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `sidekick model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    print("Anthropic-style API endpoints.  Sidekick will probe your")
...
def _model_flow_copilot(config, current_model=""):
    ...
    from cli.config import save_env_value, load_config, save_config
    from cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Sidekick turns to `copilot --acp`.")
    print("  Sidekick currently starts its own ACP subprocess for each request.")
    print("  Sidekick uses your selected model as a hint for the Copilot ACP session.")
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``sidekick setup`` / ``sidekick model``.
    ...
    recover from a malformed paste without editing ``~/.sidekick/.env`` by hand.
    ...
    from cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `sidekick setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so sidekick knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        try:
            from runtime.gemini_native_adapter import probe_gemini_tier
        except Exception:
    ...
    if provider_id == "lmstudio":
        ...
        from cli.models import fetch_lmstudio_models
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'sidekick model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Sidekick CLI with a provider."""
    from cli.auth import login_command
...
def cmd_logout(args):
    ...
    from cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from cli.auth_commands import auth_command
...
def cmd_cron(args):
    """Cron job management."""
    from cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``sidekick slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: sidekick slack <subcommand>\n"            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `sidekick slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from cli.kanban import kanban_command
...
def cmd_debug(args):
    ...
    from cli.debug import run_debug
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        ...
        from cli.backup import run_backup
...
def cmd_version(args):
    """Show version."""
    print(f"Sidekick Agent v{__version__} ({__release_date__})")
    ...
    try:
        from cli.banner import check_for_updates
        from cli.config import recommended_update_command
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``sidekick update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``sidekick_cli/web_dist/`` (per vite.config.ts
    outDir: "../sidekick_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "sidekick_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``sidekick update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``sidekick web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (sidekick web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``sidekick dashboard`` processes other than ourselves.

    ``sidekick dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``sidekick update`` replaces files on disk, the running
    ...
    patterns = [
        "sidekick dashboard",
        "sidekick_cli.main dashboard",
        "sidekick_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `sidekick update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``sidekick update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``sidekick update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: sidekick curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``sidekick dashboard`` processes.

    Called at the end of ``sidekick update`` (default ``reason``) and also
    from ``sidekick dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    sidekick dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Sidekick Agent by downloading a ZIP archive.
    ...
    branch = "master"
    zip_url = (
        f"https://github.com/Loggableim/sidekick-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="sidekick-update-")
        zip_path = os.path.join(tmp_dir, f"sidekick-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to sidekick-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"sidekick-agent-{branch}")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Sidekick behaves unexpectedly.")
    ...
    print("  Review `git diff` / `git status` if Sidekick behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `sidekick update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/Loggableim/sidekick-agent.git",
    "git@github.com:Loggableim/sidekick-agent.git",
    "https://github.com/Loggableim/sidekick-agent",
    "git@github.com:Loggableim/sidekick-agent",
}
OFFICIAL_REPO_URL = "https://github.com/Loggableim/sidekick-agent.git"
...
def _resolve_remote_default_branch(git_cmd: list[str], cwd: Path, remote: str) -> str:
    ...
    try:
        result = subprocess.run(
            git_cmd + ["symbolic-ref", "--quiet", "--short", f"refs/remotes/{remote}/HEAD"],
            cwd=cwd,
            capture_output=True,
            text=True,
        )
    ...
    try:
        ...
        prefix = f"{remote}/"
        if result.returncode == 0 and upstream.startswith(prefix):
            branch = upstream[len(prefix):].strip()
            if branch:
                return branch
    except Exception:
        pass
    ...
    for branch in ("master", "main"):
        try:
            result = subprocess.run(
                git_cmd + ["rev-parse", "--verify", "--quiet", f"refs/remotes/{remote}/{branch}"],
                cwd=cwd,
                capture_output=True,
                text=True,
            )
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_sidekick_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_sidekick_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass


def _sync_fork_with_upstream(git_cmd: list[str], cwd: Path, branch: str) -> bool:
    """Attempt to push the updated branch to origin (sync fork).
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    - Compare origin's default branch with upstream's default branch
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Sidekick repository.")
        print("  This means you may miss updates from Loggableim/sidekick-agent.")
        print()
        try:
            response = (
    ...
    origin_branch = _resolve_remote_default_branch(git_cmd, cwd, "origin")
    upstream_branch = _resolve_remote_default_branch(git_cmd, cwd, "upstream")
    ...
    if origin_ahead > 0:
        ...
        print(f"    git pull upstream {upstream_branch}")
        return
    ...
    try:
        subprocess.run(
            git_cmd + ["pull", "--ff-only", "upstream", upstream_branch],
            cwd=cwd,
            check=True,
        )
    except subprocess.CalledProcessError:
    ...
    print("→ Syncing fork...")
    if _sync_fork_with_upstream(git_cmd, cwd, origin_branch):
...
def _invalidate_update_cache():
    ...
    ``sidekick update``, every profile is now current.
    """
    homes = []
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``sidekick update`` is still progressing even if pip/uv itself is silent.
    """
...
class _UpdateOutputStream:
    """Stream wrapper used during ``sidekick update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.sidekick/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``sidekick update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``sidekick update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.sidekick/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``sidekick update --gateway``) the update is already
    ...
    try:
        ...
        # sidekick_cli.config.get_sidekick_home to simulate setup failure.
        from cli.config import get_sidekick_home as _get_sidekick_home

        logs_dir = _get_sidekick_home() / "logs"
...
def _cmd_update_check():
    ...
    compare_name = _resolve_remote_default_branch(git_cmd, PROJECT_ROOT, compare_remote)
    compare_branch = f"{compare_remote}/{compare_name}"
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``sidekick update`` without requiring a reinstall.  The
    ...
    path_comment = (
        "# Sidekick Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    ...
    try:
        from cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_sidekick_home so the user sees ~/.sidekick/...
    try:
        from runtime._compat.shim_constants import get_sidekick_home, display_sidekick_home

        home = get_sidekick_home()
        try:
            display_path = f"{display_sidekick_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def cmd_update(args):
    """Update Sidekick Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Sidekick Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  git clone https://github.com/Loggableim/sidekick-agent.git && cd sidekick-agent && python -m pip install -e ."
            )
            sys.exit(1)
    ...
    try:
        ...
        # Always update against the origin default branch.
        branch = _resolve_remote_default_branch(git_cmd, PROJECT_ROOT, "origin")

        # If user is on a non-default branch or detached HEAD, switch first.
        ...
        try:
            from cli.backup import create_quick_snapshot
        ...
        # Fork upstream sync logic for the origin default branch.
        if is_fork:
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: sidekick config migrate")
            else:
                print()
                print("Skipped. Run 'sidekick config migrate' later to configure.")
        else:
        ...
        # check `sidekick curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``sidekick update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from cli.tools_config import install_cua_driver
        ...
        # When running as ``sidekick update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_sidekick_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            try:
                from runtime._compat.shim_constants import (
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            # path in `sidekick gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: sidekick gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: sidekick -p <profile> gateway run  for each profile)"
                        )
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("    sidekick gateway migrate-legacy")
                print()
        ...
        print("  sidekick model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``sidekick -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `sidekick profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_sidekick_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → sidekick -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  sidekick profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     sidekick -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `sidekick profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from cli.profiles import profile_exists
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec sidekick -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from cli.profiles import rename_profile
        ...
        from cli.profiles import export_profile
        ...
        from cli.profiles import import_profile
        ...
        import tempfile
        from cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  sidekick -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      sidekick -p {plan.manifest.name} chat")
        ...
        from cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `sidekick profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"sidekick -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _report_dashboard_status() -> int:
    """Print ``sidekick dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``sidekick dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No sidekick dashboard processes running.")
        return 0

    print(f"{len(pids)} sidekick dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No sidekick dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `sidekick update`.
    ...
    web_dist_env = os.environ.get("SIDEKICK_WEB_DIST")
    ...
    if getattr(args, "skip_build", False):
        _dist_root = (
Path(os.environ.get("SIDEKICK_WEB_DIST"))
            if os.environ.get("SIDEKICK_WEB_DIST")
    ...
    embedded_chat = args.tui or os.environ.get("SIDEKICK_DASHBOARD_TUI") == "1" == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from cli.completion import generate_bash, generate_zsh, generate_fish
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``sidekick -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``sidekick_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``sidekick -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``sidekick`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for sidekick CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from cli.stdio import configure_windows_stdio
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: sidekick-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `sidekick model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Sidekick Agent with an interactive wizard. "
        "Run a specific section: sidekick setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'sidekick setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Sidekick.",
    )
    ...
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Sidekick")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Sidekick CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: sidekick-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Sidekick with Spotify via PKCE"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Sidekick Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.sidekick/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.sidekick/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.sidekick/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.sidekick/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Sidekick Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Sidekick setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Sidekick Agent. Use 'sidekick debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    sidekick debug share              Upload debug report and print URL
    sidekick debug share --lines 500  Include more log lines
    sidekick debug share --expire 30  Keep paste for 30 days
    sidekick debug share --local      Print report locally (no upload)
    sidekick debug share --no-redact  Disable upload-time secret redaction
    sidekick debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'sidekick debug share'",
    )
    ...
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/sidekick-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.sidekick/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo sidekick uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Sidekick backup from a zip file",
        description="Extract a previously created Sidekick backup into your "
        "Sidekick home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Sidekick Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.sidekick/skills/.bundled_manifest) "
            "so future 'sidekick update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/sidekick-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `sidekick plugins enable <name>`",
    )
    ...
    # subcommand — ``sidekick --help``, ``sidekick version``, ``sidekick logs``,
    ...
    try:
        from cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_sidekick_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_sidekick_home()}/memories/\n"
                )
                return
            ...
            from cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'sidekick tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # sidekick tools list [--platform cli]
    ...
    # sidekick tools disable <name...> [--platform cli]
    ...
    # sidekick tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from cli.tools_config import tools_disable_enable_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `sidekick computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `sidekick tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: sidekick computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: sidekick computer-use install")
            return
    ...
    # flag is omitted, causing `sidekick mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_sidekick_home() / "sessions"
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.sidekick/ (by default a "
        "single restore-point archive is written to ~/.sidekick/backups/ "
        "before apply; restorable with 'sidekick import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Sidekick Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'sidekick config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Sidekick Agent",
        description="Remove Sidekick Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Sidekick Agent as an ACP (Agent Client Protocol) server",
        description="Start Sidekick Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Sidekick Agent as an ACP server."""
        try:
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `sidekick update` skill sync)",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Sidekick Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `sidekick dashboard` cmdlines and SIGTERM them directly — the same
    # path `sidekick update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running sidekick dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running sidekick dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        ...
        epilog="""\
Examples:
    sidekick logs                    Show last 50 lines of agent.log
    sidekick logs -f                 Follow agent.log in real time
    sidekick logs errors             Show last 50 lines of errors.log
    sidekick logs gateway -n 100     Show last 100 lines of gateway.log
    sidekick logs --level WARNING    Only show WARNING and above
    sidekick logs --session abc123   Filter by session ID
    sidekick logs --component tools  Only show tool-related lines
    sidekick logs --since 1h         Lines from the last hour
    sidekick logs --since 30m -f     Follow, starting from 30 min ago
    sidekick logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``sidekick -c Pokemon Agent Dev`` → ``sidekick -c 'Pokemon Agent Dev'``
    ...
    from cli.config import get_container_exec_info
    ...
    # fails (e.g. 'sidekick -c model' where 'model' is consumed as the
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from cli.plugins import discover_plugins
        ...
        try:
            from cli.config import load_config
            from runtime.shell_hooks import register_from_config
