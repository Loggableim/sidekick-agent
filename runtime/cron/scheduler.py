...
Uses a file-based lock (~/.sidekick/cron/.tick.lock) so only one tick
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``sidekick tools`` config for the ``cron`` platform.
...
def _plugin_cron_env_var(platform_name: str) -> str:
    ...
    """
    try:
        from runtime._compat.shim_cli.plugins import discover_plugins
...
def _get_script_timeout() -> int:
    ...
    env_value = (os.getenv("SIDEKICK_CRON_SCRIPT_TIMEOUT", "")).strip()
    if env_value:
        try:
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        ...
        timeout_override: Per-job timeout in seconds from the job's
            ...
            this takes precedence over the global script timeout.
            ``_get_script_timeout()`` is used as fallback.
    ...
    script_timeout = timeout_override if (timeout_override is not None and timeout_override > 0) else _get_script_timeout()
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    if script_path:
        if prerun_script is not None:
            ...
            success, script_output = _run_job_script(script_path, timeout_override=job.get("timeout"))
        if success:
            if script_output:
                prompt = (
                    "## Script Output\n"
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        try:
            ok, output = _run_job_script(script_path, timeout_override=job.get("timeout"))
        finally:
    ...
    script_path = job.get("script")
    if script_path:
        prerun_script = _run_job_script(script_path, timeout_override=job.get("timeout"))
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_sidekick_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_sidekick_home() / ".env"), override=True, encoding="latin-1")
        ...
        model = job.get("model") or os.getenv("SIDEKICK_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_sidekick_home() / "config.yaml")
        ...
        try:
            from runtime._compat.shim_constants import apply_ipv4_preference
        ...
        prefill_file = (os.getenv("SIDEKICK_PREFILL_MESSAGES_FILE")) or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _get_sidekick_home() / pfpath
            if pfpath.exists():
                try:
        ...
        _raw_cron_timeout = (os.getenv("SIDEKICK_CRON_TIMEOUT", "")).strip()
