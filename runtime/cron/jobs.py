...
Jobs are stored in ~/.sidekick/cron/jobs.json
Output is saved to ~/.sidekick/cron/output/{job_id}/{timestamp}.md
"""
...
SIDEKICK_HOME_DIR = get_sidekick_home().resolve()
CRON_DIR = SIDEKICK_HOME_DIR / "cron"
...
# Retention for per-job output files. save_job_output writes one timestamped
# .md per run and nothing pruned them: a live install accumulated 119,857
# files over three months (one job dir at 21,661), which degrades NTFS
# directory performance. Keep the newest N files per job.
_JOB_OUTPUT_RETENTION = 200
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    try:
        ...
        run_at = _sidekick_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Sidekick configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Sidekick timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _sidekick_now().tzinfo
    if dt.tzinfo is None:
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall sidekick-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def save_jobs(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _sidekick_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.sidekick/scripts/; ``.sh`` / ``.bash`` files run via bash,
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _sidekick_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _sidekick_now().isoformat(),
        },
    )
...
def save_job_output(job_id: str, output: str):
    ...
    written = _write_unique_job_output(output_file, output)
    _prune_job_output_dir(job_output_dir)
    return written


def _prune_job_output_dir(job_output_dir: Path) -> None:
    """Delete the oldest output files beyond _JOB_OUTPUT_RETENTION.

    Best-effort: a locked or vanished file must never fail the job's save.
    """
    try:
        files = [p for p in job_output_dir.iterdir() if p.is_file()]
        if len(files) <= _JOB_OUTPUT_RETENTION:
            return
        files.sort(key=lambda p: p.stat().st_mtime, reverse=True)
        removed = 0
        for stale in files[_JOB_OUTPUT_RETENTION:]:
            try:
                stale.unlink()
                removed += 1
            except OSError:
        ...
        if removed:
            logger.info(
                "Pruned %d old output files for job dir %s (retention %d)",
                removed, job_output_dir.name, _JOB_OUTPUT_RETENTION,
            )
    except OSError:
        logger.debug("Job output pruning skipped for %s", job_output_dir, exc_info=True)
