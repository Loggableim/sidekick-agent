...
Sidekick keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for Sidekick's multi-turn
...
from runtime.gemini_schema import sanitize_gemini_tool_parameters
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with Sidekick)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Sidekick typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Sidekick retry/error classification."""
...
def _translate_tool_call_to_gemini(tool_call: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if thought_signature:
        ...
        # Gemini 2.5 thinking models require thoughtSignature on every
        # functionCall part, even when no real signature exists. Without
        # this sentinel, the API returns HTTP 400 ("Function call is
        # missing a thought_signature"). The skip sentinel tells the
        # API not to validate — mirroring the cloudcode adapter's approach.
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.sidekick/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `sidekick setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "sidekick-agent (gemini-native)",
        }
