...
The real AIAgent (15,549 LOC in cids-sidekick-agent/run_agent.py) will be
...
# Re-export _sidekick_home from the real run_agent (lazy, to avoid circular imports)
# The real run_agent.py uses _sidekick_home, not _sidekick_home. Alias it.
_sidekick_home = None
try:
    import run_agent as _real_run_agent
    _sidekick_home = getattr(_real_run_agent, '_sidekick_home', None)
    if _sidekick_home is None:
        _sidekick_home = getattr(_real_run_agent, '_sidekick_home', None)
except Exception:
    pass
