...
Usage:
    sidekick pairing list              # Show all pending + approved users
    sidekick pairing approve <platform> <code>  # Approve a pairing code
    sidekick pairing revoke <platform> <user_id> # Revoke user access
    sidekick pairing clear-pending     # Clear all expired/pending codes
"""
...
def pairing_command(args):
    ...
    if action == "list":
        ...
        print("Usage: sidekick pairing {list|approve|revoke|clear-pending}")
        print("Run 'sidekick pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.sidekick/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'sidekick pairing list' to see pending codes.\n")
